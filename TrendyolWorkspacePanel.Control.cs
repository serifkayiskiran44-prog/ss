...
public sealed partial class TrendyolWorkspacePanel
{
    ...
    FrameworkElement BuildControlToolbar()
    {
        ...
        var toolbar=new WrapPanel();var search=new StackPanel();search.Children.Add(new TextBlock{Text="Arama (Ürün adı, stok kodu, barkod)",Margin=new(3,0,0,0),FontSize=11});var searchRow=new WrapPanel();productSearch.Width=280;productSearch.MinWidth=100;searchRow.Children.Add(productSearch);searchRow.Children.Add(B("Ara",()=>{productOffset=0;RefreshProducts();}));search.Children.Add(searchRow);toolbar.Children.Add(search);
        var toggleFilters=B("⌄ Detaylı Filtreleme",()=>detailedFilters.Visibility=detailedFilters.Visibility==Visibility.Visible?Visibility.Collapsed:Visibility.Visible);toggleFilters.Name="TrendyolToggleFilters";toggleFilters.VerticalAlignment=VerticalAlignment.Bottom;toolbar.Children.Add(toggleFilters);
        var toggleBulk=B("⌄ Toplu İşlemler",()=>bulkActions.Visibility=bulkActions.Visibility==Visibility.Visible?Visibility.Collapsed:Visibility.Visible);toggleBulk.Name="TrendyolToggleBulk";toggleBulk.VerticalAlignment=VerticalAlignment.Bottom;toolbar.Children.Add(toggleBulk);
        ...
        BuildDetailedFilters();BuildBulkActions();top.Children.Add(detailedFilters);top.Children.Add(bulkActions);controlSummary.FontSize=11;controlSummary.Margin=new(5,3,5,4);top.Children.Add(controlSummary);
        var viewport=Scroll(top);viewport.MaxHeight=340;SizeChanged+=(_,_)=>viewport.MaxHeight=Math.Max(140,ActualHeight-240);return viewport;
    }
    ...
    void BuildBulkActions()
    {
        ...
        // Preserve one explicit operation selector for keyboard users and regression coverage.
        var actions=new WrapPanel();actions.Children.Add(T("İşlem"));actions.Children.Add(mode);var previewButton=Primary(B("Seçili işlemi önizle",BuildPreview));previewButton.Name="TrendyolBuildPreview";actions.Children.Add(previewButton);bulkActions.Children.Add(actions);
    }
