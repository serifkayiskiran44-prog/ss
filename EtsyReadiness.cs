...
public static class EtsyReadinessPanel
{
    public static FrameworkElement Create(string? directory, Action<string>? navigate = null)
    {
        ...
        foreach (var c in new[] { ("Kontrol", "Key", 170d), ("Durum", "Status", 100d), ("Açıklama", "Detail", 900d) }) grid.Columns.Add(GridColumns.Text(c.Item1, c.Item2, c.Item3));
        void Refresh() { var r = service.Build(); grid.ItemsSource = r.Checks; summary.Text = $"{TimeDisplay.Format(r.AtUtc)} · {r.OverallStatus}"; }
