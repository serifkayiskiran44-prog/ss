...
public static class DiagnosticsPanel
{
    ...
    public static FrameworkElement Create(string? directory, Action<string>? navigate = null, Action<Func<string, bool>>? exposeReveal = null, Func<IReadOnlyList<string>>? allowedStoreKeys = null, Action<DrillTarget>? openLink = null)
    {
        ...
        var query = new TextBox { Width = 260, ToolTip = "Audit modül, işlem, sipariş veya detay ara" }; query.TextChanged += (_, _) => events.ItemsSource = audit.List(500, query.Text); var refresh = Button(errors, "Sağlığı yenile", Refresh); var export = Button(errors, "Güvenli destek paketi dışa aktar", () => { var dialog = new SaveFileDialog { Filter = "Destek paketi (*.zip)|*.zip", FileName = ExportFileNames.Build("monobridge-destek", "zip") }; if (dialog.ShowDialog() != true) return; var started = DateTime.UtcNow; var path = SupportPackageService.Export(dialog.FileName, directory, overwrite: true); new ReportRunStore(directory).Record("support-package", started, ReportRunState.Succeeded, 0); status.Text = "Destek paketi oluşturuldu: " + path; }); var resetPreferences = Button(errors, PreferenceSchema.ResetTitle, () => { if (!DialogShell.Confirm(Window.GetWindow(panel), PreferenceSchema.ResetTitle, PreferenceSchema.ResetMessage, PreferenceSchema.ResetLabel)) return; var removed = PreferenceSchema.Reset(PreferenceSchema.OpenStore(directory)); audit.Append(new() { Module = "preferences", Action = "reset", Outcome = "OK", Detail = $"{removed} tercih kaydı silindi" }); Refresh(); status.Text = $"{removed} tercih kaydı silindi; varsayılanlar bir sonraki açılışta uygulanır."; }); var summaryFallback = new TextBox { Tag = "support-summary-fallback", IsReadOnly = true, TextWrapping = TextWrapping.Wrap, AcceptsReturn = true, MaxHeight = 160, VerticalScrollBarVisibility = ScrollBarVisibility.Auto, Visibility = Visibility.Collapsed }; System.Windows.Automation.AutomationProperties.SetName(summaryFallback, SupportSummary.ClipboardUnavailable);
        void Deliver(string text) { if (SafeClipboard.TryCopy(text)) { summaryFallback.Visibility = Visibility.Collapsed; status.Text = SupportSummary.Copied; } else { summaryFallback.Text = text; summaryFallback.Visibility = Visibility.Visible; status.Text = SupportSummary.ClipboardUnavailable; } } // #884
        var copyLast = Button(errors, "Son hatanın özetini kopyala", () => { var last = audit.LastFailure(); if (last is null) { status.Text = "Kayıtlı hata yok."; return; } Deliver(SupportSummary.ComposeFromAudit(last, CorrelationChain.IsId(last.Correlation) ? audit.ListByCorrelation(last.Correlation) : audit.List(10))); }); copyLast.Tag = "support-copy-last";
        var copySelected = Button(errors, "Seçili kaydın özetini kopyala", () => { if (events.SelectedItem is not AuditEvent row) { status.Text = "Önce bir audit kaydı seçin."; return; } Deliver(SupportSummary.ComposeFromAudit(row, CorrelationChain.IsId(row.Correlation) ? audit.ListByCorrelation(row.Correlation) : null)); }); copySelected.Tag = "support-copy-selected";
        var openSync = Button(errors, "Sync merkezine git", () => navigate?.Invoke("sync")); var openConnections = Button(errors, "Mağaza bağlantılarına git", () => navigate?.Invoke("connections"));
        ...
        panel.Children.Add(Heading("Sistem sağlık özeti")); panel.Children.Add(checks); panel.Children.Add(Heading("Audit trail")); panel.Children.Add(events); panel.Children.Add(status); panel.Children.Add(summaryFallback); Refresh(); return Scroll(panel);
    }
