...
public static class DiagnosticsPanel
{
    public static FrameworkElement Create(string? directory, Action<string>? navigate = null)
    {
        ...
        var query = new TextBox { Width = 260, ToolTip = "Audit modül, işlem, sipariş veya detay ara" }; query.TextChanged += (_, _) => events.ItemsSource = audit.List(500, query.Text); var refresh = Button(errors, "Sağlığı yenile", Refresh); var export = Button(errors, "Güvenli destek paketi dışa aktar", () => { var dialog = new SaveFileDialog { Filter = "Destek paketi (*.zip)|*.zip", FileName = "monobridge-support.zip" }; if (dialog.ShowDialog() != true) return; var started = DateTime.UtcNow; var path = SupportPackageService.Export(dialog.FileName, directory); new ReportRunStore(directory).Record("support-package", started, ReportRunState.Succeeded, 0); status.Text = "Destek paketi oluşturuldu: " + path; }); var openSync = Button(errors, "Sync merkezine git", () => navigate?.Invoke("sync")); var openConnections = Button(errors, "Mağaza bağlantılarına git", () => navigate?.Invoke("connections"));
