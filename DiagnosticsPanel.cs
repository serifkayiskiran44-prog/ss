...
public static class DiagnosticsPanel
{
    public static FrameworkElement Create(string? directory, Action<string>? navigate = null)
    {
        ...
        panel.Children.Add(Heading("Tanılama, audit trail ve destek merkezi")); panel.Children.Add(errorHost); panel.Children.Add(Hint("Sağlık özeti yerel DB/queue durumunu okur. Audit yalnız sanitized metadata saklar; destek paketi token, password, API key veya şifreli credential byte'larını içermez.")); var bar = new WrapPanel(); bar.Children.Add(new TextBlock { Text = "Audit ara", Margin = Spacing.Inline, VerticalAlignment = VerticalAlignment.Center }); bar.Children.Add(query); bar.Children.Add(refresh); bar.Children.Add(export); bar.Children.Add(openSync); bar.Children.Add(openConnections); panel.Children.Add(bar); panel.Children.Add(Heading("Sistem sağlık özeti")); panel.Children.Add(checks); panel.Children.Add(Heading("Audit trail")); panel.Children.Add(events); panel.Children.Add(status); Refresh(); return Scroll(panel);
    }
