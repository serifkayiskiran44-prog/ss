...
public sealed class OrderExceptionStore
{
    ...
    public OrderExceptionRecord Save(OrderExceptionRecord record)
    {
        ...
        record.Marketplace = record.Marketplace.Trim().ToLowerInvariant(); record.ShopId = record.ShopId.Trim(); record.OrderId = record.OrderId.Trim(); record.Type = record.Type.Trim(); record.EventKey = record.EventKey.Trim(); record.Message = MarketplaceConnectionStore.Redact(record.Message); record.UpdatedUtc = DateTime.UtcNow;
    ...
    public int Reconcile(IReadOnlyList<OrderSnapshot> orders, Catalog.CatalogStore catalog)
    {
        ...
        foreach (var order in orders)
        {
            ...
            var raw = order.RawStatus.ToLower(new CultureInfo("tr-TR")); var returned = order.Shipments.Any(x => x.State == OrdersRules.States[6]) || raw.Contains("return") || raw.Contains("iade"); var cancelled = raw.Contains("cancel") || raw.Contains("iptal");
