...
public sealed class MigrationAssistantService
{
    ...
    public MigrationPreview Preview(string path, string sourceId = "migration", CultureInfo? culture = null)
    {
        ...
        path = Path.GetFullPath(path); sourceId = string.IsNullOrWhiteSpace(sourceId) ? "migration" : sourceId.Trim(); culture ??= CultureInfo.GetCultureInfo("en-US");
    ...
    static bool Decimal(string value, CultureInfo culture, out decimal result) { var parsed = DeterministicNumberParser.Decimal(value, culture.Name, "Sayı"); result = parsed.Value; return parsed.Success; }
    static bool Int(string value, CultureInfo culture, out int result) { var parsed = DeterministicNumberParser.Integer(value, culture.Name, "Tam sayı"); result = parsed.Success && parsed.Value <= int.MaxValue ? (int)parsed.Value : 0; return parsed.Success && parsed.Value <= int.MaxValue; }
