...
public static class AllegroPanel
{
    public static FrameworkElement Create(string? directory = null)
    {
        ...
        var root = new StackPanel { Margin = Spacing.Page, MaxWidth = 820 }; root.Children.Add(new TextBlock { Text = "Allegro OAuth bağlantısı", FontSize = DesignTokens.TextSectionTitleSize, FontWeight = DesignTokens.FontWeightTitle, Margin = new Thickness(0, 0, 0, 10) });
        root.Children.Add(new TextBlock { Text = "Allegro public API ürün ve sipariş okumaları bu panelden salt okunur doğrulanır. OAuth bilgileri DPAPI ile şifrelenir; canlı yazma/fulfillment yoktur.", TextWrapping = TextWrapping.Wrap, Foreground = Brushes.DarkSlateGray, Margin = Spacing.BelowSection });
        var client = Field(root, "Client ID"); var secret = Password(root, "Client secret"); var redirect = Field(root, "Redirect URI (HTTPS)"); var token = Password(root, "Access token"); var refresh = Password(root, "Refresh token"); var state = new TextBlock { TextWrapping = TextWrapping.Wrap, Foreground = Brushes.DarkSlateGray, Margin = Spacing.VerticalControl }; root.Children.Add(state);
