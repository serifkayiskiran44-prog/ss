...
public partial class MainWindow : Window
{
 ...
 readonly ListBox sources=new(), paths=new();
 ...
 void RefreshImportStepper(){
  ...
  IReadOnlyList<ImportStep> steps;try{steps=ImportStepper.Compute(CurrentImportFlowState());}catch(Exception e){Log("Aktarım adımları hesaplanamadı: "+Safe(e));return;}
 ...
 void EvaluatePreviewRows(List<CatalogProduct> rows){
  previewRows=rows;var pool=store.Products();var bySku=pool.Where(x=>!string.IsNullOrWhiteSpace(x.Sku)).GroupBy(x=>x.Sku).ToDictionary(g=>g.Key,g=>g.First());var byBarcode=pool.Where(x=>!string.IsNullOrWhiteSpace(x.Barcode)).GroupBy(x=>x.Barcode).ToDictionary(g=>g.Key,g=>g.First());
 ...
 void BuildSources()
 {
  ...
  rowText.SetBinding(System.Windows.Automation.AutomationProperties.NameProperty,new Binding("."){Converter=new SourceRowConverter(x=>sourceRows.TryGetValue(x.Id,out var r)?r.Summary:x.Name)});
  sources.ItemTemplate=new DataTemplate{VisualTree=rowText};
  ...
  var mappingRowStyle=new Style(typeof(DataGridRow));mappingRowStyle.Setters.Add(new Setter(FrameworkElement.ToolTipProperty,new Binding("Reason")));mappingRowStyle.Setters.Add(new Setter(ToolTipService.ShowsToolTipOnKeyboardFocusProperty,true));mapping.RowStyle=mappingRowStyle;
 ...
 async Task RefreshXmlSourceHealthAsync(XmlSource candidate)
 {
  ...
  try
  {
   ...
   if(source?.Id!=id)return;
    ...
    var quarantine=new SourceMissingQuarantine(dataDirectory).List(id); var pending=quarantine.Count(x=>x.State=="PENDING_ACTION"); var warning=quarantine.Count(x=>x.State=="WARNING"); var persisted=store.Sources().FirstOrDefault(x=>x.Id==id);
 ...
 void RefreshSources(bool choose=true){
  ...
  sourceRows=rows.ToDictionary(r=>r.Id);var order=rows.Select((r,i)=>(r.Id,i)).ToDictionary(x=>x.Id,x=>x.i);
  sources.ItemsSource=all.OrderBy(x=>order.TryGetValue(x.Id,out var i)?i:int.MaxValue).ToList();
  ...
  async Task PreviewAsync(){var s=CurrentSource();if(xml==""||loadedLocation!=s.Location)throw new InvalidOperationException("Bu kaynak adresi için önce XML'i oku.");previewRevision="";previewMappingShapeFingerprint="";preview.ItemsSource=null;previewStatus.Text="Fiyat ve kur hesaplanıyor…";await UpdateFxAsync(s);BindSource();var snapshot=Clone(s);var snapshotJson=JsonSerializer.Serialize(snapshot);var mappingSnapshot=await Task.Run(()=>XmlCatalog.MappingSnapshot(xml,snapshot));XmlCatalog.EnsureMappingReady(snapshot,mappingSnapshot,scheduled:false);var rows=await Task.Run(()=>XmlCatalog.Preview(xml,snapshot));EvaluatePreviewRows(rows);previewRevision=XmlPreviewFingerprint.Create(xml,snapshotJson);previewMappingShapeFingerprint=mappingSnapshot.Fingerprint;previewStatus.Text=$"{rows.Count} ürün • Fiyat/stok hesaplandı. Eşleme revizyonu {snapshot.MappingRevision}. Kaydetmeden önce satırları seç. Etsy'ye gönderim yapılmaz.";Log($"XML önizlemesi: {rows.Count} ürün.");RefreshImportStepper();}
  async Task ImportAsync(){var s=CurrentSource();var snapshotJson=JsonSerializer.Serialize(s);if(previewRevision==""||previewRevision!=XmlPreviewFingerprint.Create(xml,snapshotJson))throw new InvalidOperationException("XML veya eşleme ayarları değişti; önizleme geçersiz. Yeniden oku ve önizle.");var mappingSnapshot=await Task.Run(()=>XmlCatalog.MappingSnapshot(xml,s));XmlCatalog.EnsureMappingReady(s,mappingSnapshot,scheduled:false);if(previewMappingShapeFingerprint!=mappingSnapshot.Fingerprint)throw new InvalidOperationException("XML yapısı önizlemeden sonra değişti; yeniden önizleyin.");CatalogPricing.ValidateRate(s);var rows=preview.SelectedItems.Cast<CatalogProduct>().Select(Clone).ToList();if(rows.Count==0)throw new InvalidOperationException("Önizlemeden en az bir ürün seç.");var complete=rows.Count==preview.Items.Count;var feedHash=complete?XmlPreviewFingerprint.FeedHash(xml):"";XmlAuthStore.Save(s.Id,new(xmlUser.Text,xmlPassword.Password),dataDirectory);store.SaveSource(s);var runs=new XmlRunStore(dataDirectory);var run=runs.Start(s.Id,feedHash,TimeSpan.FromMinutes(10));importRunning=true;importCancelled=false;importFailed=false;RefreshImportStepper();try{runs.Heartbeat(run,TimeSpan.FromMinutes(10));var context=new XmlImportContext{FeedHash=feedHash,MappingShapeFingerprint=mappingSnapshot.Fingerprint,PreviewFingerprint=previewRevision,CompleteFeed=complete,AllowMappingRevisionChange=true,ObservedAtUtc=DateTimeOffset.UtcNow};var result=await Task.Run(()=>store.Import(s,rows,lifetime.Token,context));runs.Complete(run,result);s.LastRunUtc=DateTime.UtcNow;s.LastStatus=$"{result.Added} yeni / {result.Updated} güncel / {result.Unchanged} aynı";store.SaveSource(s);previewRevision="";RefreshSources(false);_ = RefreshXmlSourceHealthAsync(s);RefreshProducts();Log(s.LastStatus);Navigate("products");}catch(OperationCanceledException){importCancelled=true;runs.Fail(run,"İptal edildi");throw;}catch(Exception e){importFailed=true;runs.Fail(run,e.Message);throw;}finally{importRunning=false;RefreshImportStepper();}}
 void ShowProducts(CatalogPage page){var baseline=productEditBaseline;if(!ResolveProductEdit())return;if(baseline!=productEditBaseline)page=store.Search(search.Text.Trim(),productOffset,200,productFilter);var id=edit?.Id;productTotal=page.Total;changingProductSelection=true;try{products.ItemsSource=page.Items;if(id!=null)products.SelectedItem=page.Items.FirstOrDefault(p=>p.Id==id);}finally{changingProductSelection=false;}BindProductEdit(products.SelectedItem as CatalogProduct);SummaryText.Text=$"{page.Total:N0} sonuç   •   {page.InStock:N0} stokta   •   {page.Linked:N0} Etsy ile eşleşen   •   Sayfa {productOffset/200+1} / {Math.Max(1,(page.Total+199)/200)}";}
