...
public partial class MainWindow : Window
{
 ...
 readonly BusyState shellBusy=new(); // #885: the overlay renders from this alone
 ...
 readonly StackPanel previewEmptyHost=new(){Visibility=Visibility.Collapsed}, sourceEmptyHost=new(){Visibility=Visibility.Collapsed}; // #886
 ...
 public MainWindow(string? directory, HttpClient? httpClient = null)
 {
  ...
  InitializeComponent();ShellLayers.Children.Add(BusyOverlay.Create(shellBusy));FontFamily=DesignTokens.FontFamilyBody;FontSize=DesignTokens.TextBodySize;IconStyles.ApplyIconButton(BackButton,IconRole.Inline);BackButton.ToolTip = KeyboardShortcuts.Hint("Geri", "back");IconStyles.ApplyIconButton(ForwardButton,IconRole.Inline);ForwardButton.ToolTip = KeyboardShortcuts.Hint("İleri", "forward"); GlobalSearchBox.ToolTip = KeyboardShortcuts.Hint("Genel arama", "global-search"); ShortcutsButton.ToolTip = KeyboardShortcuts.Hint("Klavye kısayolları", "shortcut-reference"); IconStyles.ApplyIconButton(ShortcutsButton, IconRole.Inline);RefreshBackButton();latencyStore=new LatencyStore(dataDirectory);uiPreferences=PreferenceSchema.OpenStore(directory);WindowGeometry.Restore(this, uiPreferences);Language=System.Windows.Markup.XmlLanguage.GetLanguage(CultureInfo.CurrentCulture.IetfLanguageTag);
 ...
 static void Label(Panel panel,string text,UIElement control){var label=new TextBlock{Text=text,Margin=new Thickness(4,7,4,0)};panel.Children.Add(label);panel.Children.Add(control);FormField.Labelled(label,control);}
 ...
 void BuildProducts()
 {
  ...
  productEditor.Children.Add(productPriceSummaryPanel);productProvenanceExpander.Content=productProvenanceBody;productEditor.Children.Add(productProvenanceExpander);Field(productEditor,"Alış fiyatı","Cost").IsReadOnly=true;Field(productEditor,"Alış para birimi","CostCurrency").IsReadOnly=true;Field(productEditor,"Satış fiyatı","Price");Field(productEditor,"Satış para birimi","Currency").IsReadOnly=true;Field(productEditor,"KDV oranı (%)","VatRate");var taxClass=new ComboBox{ItemsSource=ProductTaxClass.Choices,DisplayMemberPath="Label",SelectedValuePath="Code"};taxClass.SetBinding(ComboBox.SelectedValueProperty,new Binding("TaxClass"){Mode=BindingMode.TwoWay});Label(productEditor,"Vergi sınıfı",taxClass);productEditor.Children.Add(Hint("Sınıf seçiliyse KDV oranı sınıftan gelir; boş bırakılırsa fiyat hesabı mağaza kuralının KDV oranını kullanır.")); /* #909 */Field(productEditor,"Desi (kargo)","Desi");productEditor.Children.Add(Hint("Boş bırakılırsa boyuttan hesaplanır (U × G × Y / 3000) ve boyutu izler; elle girilen desi korunur.")); /* #908 */productEditor.Children.Add(Hint("XML güncellemesinde korunmasını istediğin alanı kilitle."));
  BuildPriceFieldsPanel(productEditor);
  Field(productEditor,"Ürün stok kodu / SKU","Sku").IsReadOnly=true;Field(productEditor,"Barkod","Barcode").IsReadOnly=true;Field(productEditor,"GTIN","Gtin").IsReadOnly=true;Field(productEditor,"Marka","Brand");Field(productEditor,"Kategori","Category");Field(productEditor,"Başlık","Name");Flag(productEditor,"Başlığı kilitle","LockName");Field(productEditor,"Açıklama","Description",90);Flag(productEditor,"Açıklamayı kilitle","LockDescription");Flag(productEditor,"Fiyatı kilitle","LockPrice");productEditor.Children.Add(productStockSummaryPanel);Field(productEditor,"Stok","Stock");Flag(productEditor,"Stoğu kilitle","LockStock");Field(productEditor,"Görsel URL'leri","ImageUrls",65);Flag(productEditor,"Görselleri kilitle","LockImages");Label(productEditor,"Kilit gerekçesi (kilit açıp kapatırken kaydedilir)",productLockReason);
  ...
  Field(productEditor,"Üretici parça kodu / MPN","Mpn").MaxLength=128;Field(productEditor,"Menşei ülke (kod veya ad, örn. TR / Almanya)","CountryOfOrigin").MaxLength=64; /* #910 */Field(productEditor,"Özellikler (ad=değer; noktalı virgülle ayır, örn. Beden=M; Renk=Kırmızı)","AttributesText").MaxLength=1000; /* #915 */
 ...
 async Task WriteProductsXmlAsync(string path,bool overwrite)
 {
  ...
  ExportFiles.Write(path,overwrite,CancellationToken.None,temporary=>File.WriteAllText(temporary,xml));
 ...
 // #886: the preview grid's empty state -- nothing previewed yet, or a filter hiding every row; the source list's -- no source at all, with the page's own add.
 void RenderPreviewEmptyState(){EmptyStatePanel.Render(previewEmptyHost,previewRows.Count==0?(xml.Length==0?null:EmptyState.Preview(false)):preview.Items.Count==0?EmptyState.Preview(true):null,key=>Navigate(key));}
 void RenderSourceEmptyState(){if(sourceEmptyHost.Parent==null&&sources.Parent is Panel host)host.Children.Insert(host.Children.IndexOf(sources)+1,sourceEmptyHost);EmptyStatePanel.Render(sourceEmptyHost,sources.Items.Count==0?EmptyState.Sources():null,key=>Navigate(key),_=>AddSourceOfSelectedType());}
 void RefreshImportStepper(){RenderPreviewEmptyState();
 ...
 void RefreshMappingTable(){
  ...
  mapping.Items.Refresh();mappingSummary.Text=mappingSampleTruncated?view.Summary+" · "+ImportSampleViewer.ItemTruncatedNote:view.Summary;mappingSummary.Foreground=SeverityStyle.AccentBrush(view.HasBlocking?SeverityLevel.Blocking:SeverityLevel.Success,SeverityStyle.IsHighContrast);
 ...
 void ApplyPreviewFilter(){if(applyingPreviewFilter||previewValidation.Count==0)return;var state=CurrentPreviewFilter();
  ...
  preview.ItemsSource=indices.Select(i=>previewRows[i]).ToList();RenderPreviewEmptyState();
 ...
 readonly ImportProgressState importProgress=new(); readonly StackPanel importProgressPanel=new(){Margin=new Thickness(2,0,2,8)}; readonly TextBlock importCancelOutcome=new(){Tag=CancellationOutcome.Tag,TextWrapping=TextWrapping.Wrap,Visibility=Visibility.Collapsed,VerticalAlignment=VerticalAlignment.Center,Margin=new Thickness(8,0,0,0)}; IProgress<ImportProgressEvent>? importProgressReporter; CancellationTokenSource? importCts;
 ...
 // #892: the probe carries the saved credential (presence decided elsewhere; an unreadable blob sends nothing) and a refusal is written back to the persisted source so the list and a restart show it.
 XmlAuth? ProbeAuth(XmlSource s){try{var a=XmlAuthStore.Load(s.Id,dataDirectory);return SourceCredentialHealth.PresenceOf(a)==CredentialPresence.Saved?a:null;}catch(Exception){return null;}}
 void PersistCredentialState(XmlSource s){try{var persisted=store.Sources().FirstOrDefault(x=>x.Id==s.Id);if(persisted!=null&&persisted.LastCredentialState!=s.LastCredentialState){persisted.LastCredentialState=s.LastCredentialState;store.SaveSource(persisted);}RefreshSources(false);}catch(Exception error){Log("Kimlik bilgisi durumu kaydedilemedi: "+AuditStore.Redact(error.Message));}}
 // #894: every feed a source downloads is kept under the retention policy; a failure here never fails the read. The sweep runs with the scheduler, leaving a source with a run in progress alone.
 void CacheFeed(XmlSource s,string feed,FeedCacheOutcome outcome,string runId){try{FeedCache.Store(dataDirectory,s.Id,feed,outcome,runId,s.Location);}catch(Exception error){Log("Besleme önbelleğe yazılamadı: "+AuditStore.Redact(error.Message));}}
 void SweepFeedCache(){try{var runs=new XmlRunStore(dataDirectory);var active=runs.List(null,500).Where(r=>r.Status=="Running").Select(r=>r.SourceId).Distinct().ToList();var report=FeedCache.Sweep(dataDirectory,store.Sources().Select(x=>x.Id),active);if(report.Evicted+report.Corrupt+report.Orphans>0)Log($"Besleme önbelleği temizlendi: {report.Evicted} eski, {report.Corrupt} bozuk, {report.Orphans} sahipsiz kayıt kaldırıldı.");}catch(Exception error){Log("Besleme önbelleği temizlenemedi: "+AuditStore.Redact(error.Message));}}
 CancellationToken BeginImportStage(){importProgress.BeginOperation();importCts?.Dispose();importCts=CancellationTokenSource.CreateLinkedTokenSource(lifetime.Token);return importCts.Token;}
 void RenderImportProgress(){
  ...
  // #890: the outcome line comes from the real state -- requested, stopping at a safe point, cancelled, or completed before the request took effect -- and the button waits once it has asked.
  var verdict=CancellationOutcome.Describe(importProgress.CancellationFacts(now),"Havuz değişmedi; yazılan satır yok.");
  importCancelOutcome.Text=verdict.Line;importCancelOutcome.Visibility=verdict.Phase==CancellationPhase.NotRequested?Visibility.Collapsed:Visibility.Visible;importCancelOutcome.Foreground=SeverityStyle.AccentBrush(verdict.Level,SeverityStyle.IsHighContrast);
  CommandState.Apply(importCancelButton,importProgress.CancelRequestedUtc==null||verdict.IsTerminal?null:DisabledReason.Busy("İptal istendi; yanıt bekleniyor."));}
 ...
 async Task ScanSourceHealthAsync(){
  ...
  try{using var limit=new SemaphoreSlim(4);
   await Task.WhenAll(targets.Select(async s=>{await limit.WaitAsync(lifetime.Token);try{var health=await XmlSourceHealthChecker.CheckAsync(http,s,lifetime.Token,TimeSpan.FromSeconds(10),ProbeAuth(s));
    await Dispatcher.InvokeAsync(()=>{s.LastHealthCheckUtc=health.CheckedUtc;s.LastHealthState=health.State;s.LastCredentialState=SourceCredentialHealth.Evaluate(XmlAuthStore.Presence(s.Id,dataDirectory),health.HttpStatus,health.State);s.LastHealthHttpStatus=health.HttpStatus;s.LastHealthLatencyMs=health.LatencyMs;s.LastHealthError=MarketplaceConnectionStore.Redact(health.ErrorMessage);store.SaveSource(s);if(health.State!="HEALTHY"||health.LatencyMs>=XmlSourceListGrouping.DegradedLatencyMs)problems++;done++;sourceScanStatus.Text=$"Sağlık taraması: {done}/{targets.Count} · {problems} sorunlu";});}
 ...
 async Task CheckSourceReachabilityAsync(){
  ...
  var health=await XmlSourceHealthChecker.CheckAsync(http,candidate,lifetime.Token,TimeSpan.FromSeconds(10),ProbeAuth(candidate));
  void Record(XmlSource s){s.LastHealthCheckUtc=health.CheckedUtc;s.LastHealthState=health.State;s.LastCredentialState=SourceCredentialHealth.Evaluate(XmlAuthStore.Presence(s.Id,dataDirectory),health.HttpStatus,health.State);s.LastHealthHttpStatus=health.HttpStatus;s.LastHealthLatencyMs=health.LatencyMs;s.LastHealthError=MarketplaceConnectionStore.Redact(health.ErrorMessage);}
 ...
 void BuildSources()
 {
  ...
  Flag(sourceGeneral,"Kaynak aktif","Enabled");Flag(sourceGeneral,"Program açıkken otomatik havuz güncellemesi","AutoImport");Field(sourceGeneral,"Kontrol aralığı (dakika)","IntervalMinutes");Field(sourceGeneral,"Öncelik (yüksek kazanır; eşitse daha yeni besleme)","Priority");Field(sourceGeneral,"Güncellik SLA · beklenen yenileme (dk, 0 = kontrol aralığı)","SlaRefreshMinutes");Field(sourceGeneral,"Güncellik SLA · tolerans (dk, 0 = yarısı, en az 15)","SlaGraceMinutes");Label(sourceGeneral,"Basic Auth kullanıcı adı (isteğe bağlı)",xmlUser);Label(sourceGeneral,"XML şifresi",xmlPassword);var healthBody=new StackPanel();healthBody.Children.Add(xmlSourceHealthHeadline);healthBody.Children.Add(xmlSourceHealthLines);healthBody.Children.Add(xmlSourceHealth);healthBody.Children.Add(xmlSourceHealthActions);xmlSourceHealthPanel.Child=healthBody;sourceGeneral.Children.Add(xmlSourceHealthPanel);sourceGeneral.Children.Add(AsyncButton("Kaynak sağlığını kontrol et (tam okuma)",CheckXmlSourceAsync));sourceGeneral.Children.Add(Hint("Şifre Windows hesabına özel şifrelenir. XML sınırı 25 MB; HTTPS, timeout, HTTP durum kodu ve gzip yanıtı denetlenir. Kaynaktan kaybolan ürünler korunur; otomatik Etsy gönderimi yapılmaz."));
  sources.Height=115;
  ...
  var selector=new FrameworkElementFactory(typeof(ComboBox));selector.SetValue(ComboBox.ItemsSourceProperty,xmlPaths);selector.SetValue(System.Windows.Automation.AutomationProperties.NameProperty,"XML alan yolu");selector.SetValue(ComboBox.IsEditableProperty,true);selector.SetValue(ComboBox.IsTextSearchEnabledProperty,true);selector.SetBinding(ComboBox.TextProperty,new Binding("Path"){Mode=BindingMode.TwoWay,UpdateSourceTrigger=UpdateSourceTrigger.PropertyChanged});mapping.RowHeight=42;
 ...
 void LoadSourceTemplate(){if(!Directory.Exists(AppPaths.TemplatesDirectory))Log("Şablon klasörü bulunamadı ("+AppPaths.TemplatesDirectory+"); EXE publish klasöründeki haliyle çalıştırılmalı. Dosyayı elle seçebilirsiniz.");var dialog=new OpenFileDialog{Filter="MarketplaceHub XML şablonu (*.json)|*.json",InitialDirectory=Directory.Exists(AppPaths.TemplatesDirectory)?AppPaths.TemplatesDirectory:""};if(dialog.ShowDialog(this)!=true)return;if(new FileInfo(dialog.FileName).Length>1024*1024)throw new InvalidOperationException("Şablon dosyası 1 MB sınırını aşıyor.");var s=JsonSerializer.Deserialize<XmlSource>(File.ReadAllText(dialog.FileName))??throw new InvalidOperationException("Şablon okunamadı.");s.Id=Guid.NewGuid().ToString("N");s.AutoImport=false;s.LastRunUtc=null;s.LastStatus="Şablondan açıldı; adresi ve eşleştirmeyi kontrol edin.";XmlCatalog.ValidateSource(s);SetSource(s);Log("XML şablonu açıldı; kaynak adresini gir ve önizle.");}
 void ExportSourceTemplate(){var s=Clone(CurrentSource());s.AutoImport=false;s.LastRunUtc=null;s.LastStatus="";var dialog=new SaveFileDialog{Filter="XML şablonu (*.json)|*.json",FileName=ExportFileNames.Build("xml-sablonu","json")};if(dialog.ShowDialog(this)==true){ExportFiles.Write(dialog.FileName,true,CancellationToken.None,temporary=>File.WriteAllText(temporary,JsonSerializer.Serialize(s,new JsonSerializerOptions{WriteIndented=true})));Log("XML alan ve kural şablonu kaydedildi. Şifreler dışa aktarılmadı.");}}
 ...
 void SetSource(XmlSource s)
 {
  ...
  var names=new[]{("Sku","SKU / stok kodu"),("Barcode","Barkod"),("Gtin","GTIN / EAN / UPC"),("Name","Ürün adı"),("Description","Açıklama"),("Cost","Alış fiyatı"),("Stock","Stok"),("Brand","Marka"),("Category","Kategori"),("ImageUrls","Görseller"),("Weight","Ağırlık (birimli: 1,5 kg / 1500 g)"),("Dimensions","Boyut (U x G x Y, birimli: 20 x 30 x 40 cm)"),("TaxClass","Vergi sınıfı (kod: standard, reduced, super-reduced, zero, exempt)"),("CountryOfOrigin","Menşei ülke (kod veya ad: TR, DE, Almanya)")};
  ...
  try
  {
   ...
   if(source?.Id!=id)return;
    ...
    var facts=new SourceHealthFacts(h.LastHealthState,h.LastHealthCheckUtc,h.LastHealthLatencyMs,h.LastHealthHttpStatus,h.LastHealthError,h.LastSuccessfulFeedUtc,h.LastSuccessfulFeedCount,h.LastFeedState,candidate.MappingRevision,h.LastAppliedMappingRevision,h.LastMappingShapeFingerprint,shapeKnown?previewMappingShapeFingerprint:null,runSnapshot,summary.count,pending,warning,
     previewIsThis?previewValidation.Count(v=>v.Highest==SeverityLevel.Blocking):null,previewIsThis?previewValidation.Count(v=>v.Highest==SeverityLevel.Warning):null,previewIsThis?previewValidation.Count:null,previewIsThis?previewEvaluatedUtc:null,importProgress.Failed,importRunning);facts=facts with{CredentialState=h.LastCredentialState,Sla=SourceSla.Evaluate(h,DateTime.UtcNow)};
 ...
 IReadOnlyList<RowAction> SourceRowActions(){var none=sources.SelectedItem is XmlSource?null:"Önce kaynak seçin.";return new RowAction[]{new("source-inspect","XML'i oku / alanları bul",()=>RunGuarded(InspectAsync),none),new("source-health","Kaynak sağlığını kontrol et (tam okuma)",()=>RunGuarded(CheckXmlSourceAsync),none),new("source-anomaly","Son anomali tanıları (engel/uyarı açıklaması)",()=>RunGuarded(ShowAnomalyDiagnosticsAsync),none),new("source-delete","Kaynağı sil (kullanılmıyorsa)",()=>RunGuarded(DeleteSourceAsync),none,true)};}
 ...
 // #939: the last anomaly decisions of the selected source in words -- values, baseline, thresholds, source age, correlation id; nothing of a product or an address.
 Task ShowAnomalyDiagnosticsAsync(){if(source==null)throw new InvalidOperationException("Önce XML kaynağı seçin.");var latest=store.AnomalyDiagnosticsFor(source.Id,5);Log(latest.Count==0?"Bu kaynak için anomali tanısı yok: içe aktarımlar engellenmedi ve eşiğe yaklaşmadı.":"Anomali tanıları (en yeni önce): "+string.Join(" | ",latest.Select(d=>$"#{d.Id} {SafetyBufferProfile.Stamp(d.DecidedUtc)} {d.Words}")));return Task.CompletedTask;}
 ...
 // #901: a guarded delete -- refused with the references and the switch-off alternative when anything still points at the source; confirmed explicitly when nothing does; the store decides again at the moment of deletion.
 Task DeleteSourceAsync(){var s=CurrentSource();var verdict=SourceDeleteGuard.Evaluate(s,store.SourceReferences(s.Id));if(!verdict.Allowed)throw new InvalidOperationException(verdict.Headline+(verdict.Alternative.Length>0?" "+verdict.Alternative:""));if(!DialogShell.Confirm(this,"Kaynağı sil",verdict.Body,"Sil",destructive:true)){Log("Kaynak silinmedi; hiçbir şey değişmedi.");return Task.CompletedTask;}var result=store.DeleteSource(s.Id);if(!result.Allowed)throw new InvalidOperationException(result.Headline+(result.Alternative.Length>0?" "+result.Alternative:""));try{new AuditStore(dataDirectory).Append(SourceDeleteGuard.ToAudit(s,result,true));}catch(Exception){}source=null;BindSource();RefreshSources(true);Log("Kaynak silindi: "+AuditStore.Redact(s.Name));return Task.CompletedTask;}
 // #899: the newest run record of a source, for the save preview; null when there is none or the history cannot be read.
 SourceRunSnapshot? LatestRunFor(string id){try{return XmlSourceListGrouping.LatestRuns(new XmlRunStore(dataDirectory).List(id,1)).TryGetValue(id,out var r)?r:null;}catch(Exception){return null;}}
 void SaveSource(){var s=CurrentSource();
  ...
  // #899: what this save would touch, before it touches anything -- a stale edit is refused with words, an impact is confirmed, a cancel changes nothing; addresses are masked and credentials are not part of the comparison.
  var impact=SourceChangeImpact.Preview(latest,s,store.Products().Count(p=>p.SourceId==s.Id),LatestRunFor(s.Id),DateTime.UtcNow);
  if(impact.Stale)throw new InvalidOperationException(impact.Headline);
  // #900: turning a source off shows what it leaves behind -- linked products, the scheduler, a running import, which products keep a fallback -- and asks explicitly; a cancel changes nothing.
  SourceDisablePreview? disable=null;
  if(SourceDisable.IsDisable(latest,s)){var linked=store.Products().Where(p=>p.SourceId==s.Id).ToList();disable=SourceDisable.Preview(latest!,s,linked,store.Sources(),store.SightingsByProduct(),LatestRunFor(s.Id),DateTime.UtcNow);if(disable.Stale)throw new InvalidOperationException(disable.Headline);if(!DialogShell.Confirm(this,"Kaynağı devre dışı bırak",disable.Body,"Devre dışı bırak",destructive:true)){Log("Kaynak devre dışı bırakılmadı; hiçbir şey değişmedi.");return;}}
  else if(impact.RequiresConfirmation&&!DialogShell.Confirm(this,"Kaynak değişikliği",impact.Body,"Kaydet",destructive:impact.Level==SourceChangeLevel.Large)){Log("Kaynak kaydı iptal edildi; hiçbir şey değişmedi.");return;}
  if(latest!=null){s.LastRunUtc=latest.LastRunUtc;s.LastStatus=latest.LastStatus;}XmlAuthStore.Save(s.Id,new(xmlUser.Text,xmlPassword.Password),dataDirectory);s.LastCredentialState=SourceCredentialHealth.AfterSave(new(xmlUser.Text,xmlPassword.Password));store.SaveSource(s);if(disable is not null){try{new AuditStore(dataDirectory).Append(SourceDisable.ToAudit(s,disable));}catch(Exception){}Log("Kaynak devre dışı bırakıldı: "+disable.Headline,disable.WithoutFallback>0?NotificationSeverity.Warning:NotificationSeverity.Info);}RefreshSources(false);_ = RefreshXmlSourceHealthAsync(s);Log("XML kaynağı, eşleştirme ve kurallar kaydedildi.");}
 void RefreshSources(bool choose=true){
  var recent=PreferenceSchema.Read(uiPreferences, ImportSourceCatalog.RecentPreferenceKey);var all=store.Sources();
  ...
  var keyboard=FocusRestore.Capture(sources,x=>((XmlSource)x).Id);changingSourceList=true;try{sources.ItemsSource=view;RenderSourceEmptyState();if(selectedId!=null)sources.SelectedItem=ordered.FirstOrDefault(x=>x.Id==selectedId);}finally{changingSourceList=false;}FocusRestore.Restore(sources,keyboard,x=>((XmlSource)x).Id);
  ...
  async Task InspectAsync(){var s=CurrentSource();xml="";loadedLocation="";previewRevision="";previewMappingShapeFingerprint="";preview.ItemsSource=null;var importToken=BeginImportStage();var latency=UiLatency.Begin(latencyStore,UiLatency.ImportView,LatencyPhase.Load,s.Id);ReportImport(ImportProgressStage.Download,ImportProgressStatus.Running);try{xml=await new XmlSourceReader(http).ReadAsync(s.Location,new(xmlUser.Text,xmlPassword.Password),importToken);}catch(OperationCanceledException){latency.Cancel();ReportImport(ImportProgressStage.Download,ImportProgressStatus.Cancelled);throw;}catch(Exception e){latency.Fail();if(e is XmlSourceAuthException refused){s.LastCredentialState=SourceCredentialHealth.AfterRefusal(refused.CredentialSent);PersistCredentialState(s);}ReportImport(ImportProgressStage.Download,ImportProgressStatus.Failed,note:Safe(e));throw;}ReportImport(ImportProgressStage.Download,ImportProgressStatus.Done,xml.Length,xml.Length,"karakter");loadedLocation=s.Location;CacheFeed(s,xml,FeedCacheOutcome.Success,"");ReportImport(ImportProgressStage.Read,ImportProgressStatus.Running);XmlScan scan;try{scan=await Task.Run(()=>XmlCatalog.Inspect(xml,string.IsNullOrWhiteSpace(s.ItemPath)?null:s.ItemPath),importToken);}catch(Exception e){latency.Fail();ReportImport(ImportProgressStage.Read,ImportProgressStatus.Failed,note:Safe(e));throw;}ReportImport(ImportProgressStage.Read,ImportProgressStatus.Done,scan.Paths.Count,scan.Paths.Count,"alan");itemPath.Text=scan.ItemPath;paths.ItemsSource=scan.Paths;xmlPaths.Clear();xmlPaths.Add("");foreach(var path in scan.Paths)xmlPaths.Add(path);foreach(var m in mappings)if(string.IsNullOrWhiteSpace(m.Path)&&scan.SuggestedFields.TryGetValue(m.Key,out var value))m.Path=value;mapping.Items.Refresh();previewRevision="";preview.ItemsSource=null;Log($"XML okundu; {scan.Paths.Count} alan yolu bulundu.");try{var sampleItem=ImportSampleViewer.ReadFirstItem(xml,scan.ItemPath);mappingSampleTruncated=sampleItem.Truncated;mappingSampleItem=sampleItem.Element??(ImportSampleViewer.IsPlainPath(scan.ItemPath)?null:System.Xml.Linq.XDocument.Parse(xml).XPathSelectElement(scan.ItemPath));}catch(Exception){mappingSampleItem=null;mappingSampleTruncated=false;}RefreshMappingTable();RefreshImportStepper();latency.Complete(scan.Paths.Count);}
  async Task PreviewAsync(){var s=CurrentSource();if(xml==""||loadedLocation!=s.Location)throw new InvalidOperationException("Bu kaynak adresi için önce XML'i oku.");previewRevision="";previewMappingShapeFingerprint="";preview.ItemsSource=null;previewStatus.Text="Fiyat ve kur hesaplanıyor…";await UpdateFxAsync(s);BindSource();var snapshot=Clone(s);var snapshotJson=JsonSerializer.Serialize(snapshot);var previewToken=BeginImportStage();var latency=UiLatency.Begin(latencyStore,UiLatency.ImportView,LatencyPhase.Refresh,s.Id);ReportImport(ImportProgressStage.Parse,ImportProgressStatus.Running);XmlMappingSnapshot mappingSnapshot;try{mappingSnapshot=await Task.Run(()=>XmlCatalog.MappingSnapshot(xml,snapshot),previewToken);}catch(Exception e){latency.Fail();ReportImport(ImportProgressStage.Parse,ImportProgressStatus.Failed,note:Safe(e));throw;}ReportImport(ImportProgressStage.Parse,ImportProgressStatus.Done,mappingSnapshot.ItemCount,mappingSnapshot.ItemCount,"ürün");ReportImport(ImportProgressStage.Validate,ImportProgressStatus.Running);try{XmlCatalog.EnsureMappingReady(snapshot,mappingSnapshot,scheduled:false);}catch(Exception e){latency.Fail();ReportImport(ImportProgressStage.Validate,ImportProgressStatus.Failed,note:Safe(e));throw;}ReportImport(ImportProgressStage.Validate,ImportProgressStatus.Done);ReportImport(ImportProgressStage.Preview,ImportProgressStatus.Running,0,mappingSnapshot.ItemCount);List<CatalogProduct> rows;try{var sink=ImportReporter();rows=await Task.Run(()=>XmlCatalog.Preview(xml,snapshot,sink),previewToken);}catch(OperationCanceledException){latency.Cancel();ReportImport(ImportProgressStage.Preview,ImportProgressStatus.Cancelled);throw;}catch(Exception e){latency.Fail();ReportImport(ImportProgressStage.Preview,ImportProgressStatus.Failed,note:Safe(e));throw;}ReportImport(ImportProgressStage.Preview,ImportProgressStatus.Done,rows.Count,rows.Count,"ürün");EvaluatePreviewRows(rows);previewRevision=XmlPreviewFingerprint.Create(xml,snapshotJson);previewMappingShapeFingerprint=mappingSnapshot.Fingerprint;previewStatus.Text=$"{rows.Count} ürün • Fiyat/stok hesaplandı. Eşleme revizyonu {snapshot.MappingRevision}. Kaydetmeden önce satırları seç. Etsy'ye gönderim yapılmaz.";Log($"XML önizlemesi: {rows.Count} ürün.");RefreshImportStepper();latency.Complete(rows.Count);}
  async Task ImportAsync(){var s=CurrentSource();var snapshotJson=JsonSerializer.Serialize(s);if(previewRevision==""||previewRevision!=XmlPreviewFingerprint.Create(xml,snapshotJson))throw new InvalidOperationException("XML veya eşleme ayarları değişti; önizleme geçersiz. Yeniden oku ve önizle.");var mappingSnapshot=await Task.Run(()=>XmlCatalog.MappingSnapshot(xml,s));XmlCatalog.EnsureMappingReady(s,mappingSnapshot,scheduled:false);if(previewMappingShapeFingerprint!=mappingSnapshot.Fingerprint)throw new InvalidOperationException("XML yapısı önizlemeden sonra değişti; yeniden önizleyin.");CatalogPricing.ValidateRate(s);var selectedFlags=preview.SelectedItems.Cast<CatalogProduct>().Select(p=>previewFlags.TryGetValue(p,out var f)?f:null).ToList();var rejectedCount=selectedFlags.Count(f=>f?.Highest==SeverityLevel.Blocking);var warningCount=selectedFlags.Count(f=>f?.Highest==SeverityLevel.Warning);var rows=preview.SelectedItems.Cast<CatalogProduct>().Select(Clone).ToList();if(rows.Count==0)throw new InvalidOperationException("Önizlemeden en az bir ürün seç.");var complete=rows.Count==preview.Items.Count;var feedHash=complete?XmlPreviewFingerprint.FeedHash(xml):"";XmlAuthStore.Save(s.Id,new(xmlUser.Text,xmlPassword.Password),dataDirectory);s.LastCredentialState=SourceCredentialHealth.AfterSave(new(xmlUser.Text,xmlPassword.Password));store.SaveSource(s);var runs=new XmlRunStore(dataDirectory);var run=runs.Start(s.Id,feedHash,TimeSpan.FromMinutes(10),s.ConfigRevision);importRunning=true;importCancelled=false;importFailed=false;importStartedUtc=DateTime.UtcNow;importCompletionPanel.Visibility=Visibility.Collapsed;RefreshImportStepper();try{runs.Heartbeat(run,TimeSpan.FromMinutes(10));var context=new XmlImportContext{FeedHash=feedHash,MappingShapeFingerprint=mappingSnapshot.Fingerprint,PreviewFingerprint=previewRevision,CompleteFeed=complete,AllowMappingRevisionChange=true,ObservedAtUtc=DateTimeOffset.UtcNow,RunId=run,SourceRevision=s.ConfigRevision};var applyToken=BeginImportStage();ReportImport(ImportProgressStage.Apply,ImportProgressStatus.Running,0,rows.Count);var applySink=ImportReporter();ImportSummary result;try{result=await Task.Run(()=>store.Import(s,rows,applyToken,context,applySink),applyToken);}catch(OperationCanceledException){ReportImport(ImportProgressStage.Apply,ImportProgressStatus.Cancelled);throw;}catch(Exception e){ReportImport(ImportProgressStage.Apply,ImportProgressStatus.Failed,note:Safe(e));throw;}ReportImport(ImportProgressStage.Apply,ImportProgressStatus.Done,rows.Count,rows.Count,$"{result.Added} yeni / {result.Updated} güncel");runs.Complete(run,result);s.LastRunUtc=DateTime.UtcNow;s.LastStatus=$"{result.Added} yeni / {result.Updated} güncel / {result.Unchanged} aynı";store.SaveSource(s);previewRevision="";RefreshSources(false);_ = RefreshXmlSourceHealthAsync(s);RefreshProducts();Log(s.LastStatus);CompleteImport(new(s.Name,s.MappingRevision,feedHash,complete,rows.Count,rejectedCount,warningCount,result,"",false,importStartedUtc,DateTime.UtcNow));}catch(OperationCanceledException){importCancelled=true;runs.Fail(run,"İptal edildi");CompleteImport(new(s.Name,s.MappingRevision,feedHash,complete,rows.Count,rejectedCount,warningCount,null,"",true,importStartedUtc,DateTime.UtcNow));throw;}catch(Exception e){importFailed=true;runs.Fail(run,e.Message);CompleteImport(new(s.Name,s.MappingRevision,feedHash,complete,rows.Count,rejectedCount,warningCount,null,Safe(e),false,importStartedUtc,DateTime.UtcNow));throw;}finally{importRunning=false;RefreshImportStepper();}}
 void ShowProducts(CatalogPage page){var baseline=productEditBaseline;if(!ResolveProductEdit())return;if(baseline!=productEditBaseline)page=store.Search(search.Text.Trim(),productOffset,200,productFilter);var selection=SelectionPersistence.Capture(products,x=>((CatalogProduct)x).Id);if(selection.Keys.Count==0&&edit?.Id is {} editId)selection=new SelectionSnapshot(new[]{editId},editId,"");productTotal=page.Total;var keyboard=FocusRestore.Capture(products,x=>((CatalogProduct)x).Id);SelectionRestoreResult restored;changingProductSelection=true;try{products.ItemsSource=page.Items;restored=SelectionPersistence.Restore(products,selection,x=>((CatalogProduct)x).Id);}finally{changingProductSelection=false;}productSelectionNote=restored.Note;BindProductEdit(products.SelectedItem as CatalogProduct);FocusRestore.Restore(products,keyboard,x=>((CatalogProduct)x).Id);UpdateProductSelectionSummary();SummaryText.Text=$"{page.Total:N0} sonuç   •   {page.InStock:N0} stokta   •   {page.Linked:N0} Etsy ile eşleşen   •   Sayfa {productOffset/200+1} / {Math.Max(1,(page.Total+199)/200)}";RenderProductEmptyState(page.Total);}
 void RefreshProducts(){globalSearchIndex.Invalidate();searchRevision++;var latency=UiLatency.Begin(latencyStore,UiLatency.ProductsView,latencyStore.PhaseFor(UiLatency.ProductsView),UiLatency.Unscoped);try{var page=store.Search(search.Text.Trim(),productOffset,200,productFilter);ShowProducts(page);latency.Complete(page.Items.Count);}catch(Exception){latency.Fail();throw;}SaveProductLayout();}
 async Task SearchProductsAsync(){var revision=++searchRevision;var q=search.Text.Trim();var offset=productOffset;var filter=productFilter;var latency=UiLatency.Begin(latencyStore,UiLatency.ProductsView,latencyStore.PhaseFor(UiLatency.ProductsView),UiLatency.Unscoped);try{var page=await Task.Run(()=>store.Search(q,offset,200,filter));if(revision==searchRevision){ShowProducts(page);latency.Complete(page.Items.Count);}else latency.Supersede();}catch(Exception e){latency.Fail();Log(Safe(e), NotificationSeverity.Error);}}
 ...
 async Task ScheduledAsync()
 {
  ...
  try{
   // #898: an overdue source is retried at its grace cadence instead of waiting a whole check interval.
   var due=store.Sources().Where(s=>s.Enabled&&s.AutoImport&&(DateTime.UtcNow-(s.LastRunUtc??DateTime.MinValue)>=TimeSpan.FromMinutes(s.IntervalMinutes)||SourceSla.DueForRetry(s,DateTime.UtcNow))).ToList();
   ...
   // #898: the refresh SLA -- a source crossing into or out of overdue is recorded once (the state word on the record, a log line, an audit row); nothing else changes here.
   foreach(var s in store.Sources()){var sla=SourceSla.Evaluate(s,DateTime.UtcNow);if(!SourceSla.Transition(s,sla))continue;try{store.SaveSource(s);}catch(SourceRevisionConflictException){continue;}Log($"Güncellik SLA · {s.Name}: {sla.Line}",sla.State==SourceSlaState.Overdue?NotificationSeverity.Warning:NotificationSeverity.Info);try{new AuditStore(dataDirectory).Append(SourceSla.ToAudit(s,sla));}catch(Exception){}}
   ...
   using var busy=shellBusy.Enter("scheduler","Zamanlanmış kontrol sürüyor…");
   SweepFeedCache();
    foreach(var s in due){var runs=new XmlRunStore(dataDirectory);string run="";try{if(SourceCredentialHealth.ShouldFailFast(s.LastCredentialState))throw new InvalidOperationException("Kimlik bilgisi "+SourceCredentialHealth.Describe(s.LastCredentialState).Word+"; zamanlı okuma yapılmadı. Kaynak ayarlarında kimlik bilgisini girip kaydedin.");var text=await new XmlSourceReader(http).ReadAsync(s.Location,XmlAuthStore.Load(s.Id,dataDirectory),lifetime.Token);var feedHash=XmlPreviewFingerprint.FeedHash(text);run=runs.Start(s.Id,feedHash,TimeSpan.FromMinutes(10),s.ConfigRevision);CacheFeed(s,text,FeedCacheOutcome.Success,run);var mappingSnapshot=await Task.Run(()=>XmlCatalog.MappingSnapshot(text,s),lifetime.Token);XmlCatalog.EnsureMappingReady(s,mappingSnapshot,scheduled:true);await UpdateFxAsync(s);var rows=await Task.Run(()=>XmlCatalog.Preview(text,s),lifetime.Token);runs.Heartbeat(run,TimeSpan.FromMinutes(10));var context=new XmlImportContext{FeedHash=feedHash,MappingShapeFingerprint=mappingSnapshot.Fingerprint,CompleteFeed=true,AllowMappingRevisionChange=false,ObservedAtUtc=DateTimeOffset.UtcNow,RunId=run,SourceRevision=s.ConfigRevision};var result=await Task.Run(()=>store.Import(s,rows,lifetime.Token,context),lifetime.Token);runs.Complete(run,result);s.LastStatus=result.AlreadyApplied?$"Otomatik: aynı feed tekrarlandı ({result.FeedHash[..Math.Min(12,result.FeedHash.Length)]})":$"Otomatik: {result.Added} yeni / {result.Updated} güncel / {result.Unchanged} aynı";}catch(Exception e){if(e is XmlSourceAuthException refused)s.LastCredentialState=SourceCredentialHealth.AfterRefusal(refused.CredentialSent);if(run!="")try{runs.Fail(run,e.Message);}catch{}s.LastStatus=Safe(e);}s.LastRunUtc=DateTime.UtcNow;store.SaveSource(s);_ = RefreshXmlSourceHealthAsync(s);Log(s.LastStatus);}
    foreach(var s in dueHealth){if(due.Any(x=>x.Id==s.Id))continue;var health=await XmlSourceHealthChecker.CheckAsync(http,s,lifetime.Token,null,ProbeAuth(s));s.LastHealthCheckUtc=health.CheckedUtc;s.LastHealthState=health.State;s.LastCredentialState=SourceCredentialHealth.Evaluate(XmlAuthStore.Presence(s.Id,dataDirectory),health.HttpStatus,health.State);s.LastHealthHttpStatus=health.HttpStatus;s.LastHealthLatencyMs=health.LatencyMs;s.LastHealthError=MarketplaceConnectionStore.Redact(health.ErrorMessage);store.SaveSource(s);if(source?.Id==s.Id)_ = RefreshXmlSourceHealthAsync(s);Log($"Kaynak sağlığı {s.Name}: {health.State}"+(health.HttpStatus.HasValue?$" (HTTP {health.HttpStatus})":"")+$" · {health.LatencyMs}ms.");}
  ...
  }catch(Exception e){Log(Safe(e), NotificationSeverity.Error);}finally{gate.Release();}
 }
 async Task RunAsync(Func<Task> action){if(!await gate.WaitAsync(0)){Log("Önceki işlem sürüyor.");return;}using var busy=shellBusy.Enter("shell",BusyLabel(UiActivity.NameOf(action)),()=>importCts?.Cancel());try{using var activity=UiActivity.Enter(UiActivity.NameOf(action));await action();}catch(Exception e){Log(Safe(e), NotificationSeverity.Error);apiStatus.Text=Safe(e);}finally{gate.Release();}}
 static string BusyLabel(string name)=>name switch{"ImportAsync"=>"Ürünler içe aktarılıyor…","PreviewAsync"=>"Önizleme hazırlanıyor…","InspectAsync"=>"XML okunuyor…","ApplyExcelAsync"=>"Excel uygulanıyor…","RenderPreviewAsync"=>"Excel önizlemesi hazırlanıyor…","CheckSourceReachabilityAsync"=>"Kaynak erişimi denetleniyor…",_=>BusyState.DefaultLabel};
