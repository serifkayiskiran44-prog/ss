...
public partial class MainWindow : Window
{
 ...
 void SaveSource(){var s=CurrentSource();
  ...
  if(impact.RequiresConfirmation&&!DialogShell.Confirm(this,"Kaynak değişikliği",impact.Body,"Kaydet",destructive:impact.Level==SourceChangeLevel.Large)){Log("Kaynak kaydı iptal edildi; hiçbir şey değişmedi.");return;}
  if(latest!=null){s.LastRunUtc=latest.LastRunUtc;s.LastStatus=latest.LastStatus;}XmlAuthStore.Save(s.Id,new(xmlUser.Text,xmlPassword.Password),dataDirectory);s.LastCredentialState=SourceCredentialHealth.AfterSave(new(xmlUser.Text,xmlPassword.Password));store.SaveSource(s);RefreshSources(false);_ = RefreshXmlSourceHealthAsync(s);Log("XML kaynağı, eşleştirme ve kurallar kaydedildi.");}
