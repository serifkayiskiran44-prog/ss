...
public partial class MainWindow : Window
{
 ...
 void ShowProductChannelStatus(CatalogProduct? product){if(product==null){productChannelSummary.Text="Ürün seçince kanal planları burada görünür.";productChannelSummaryMirror.Text=productChannelSummary.Text;return;}var rows=MarketplaceProductPanelModel.Build(product.Id,dataDirectory).Select(x=>$"{x.Channel} / {x.ShopId}: {x.Status} · {x.Readiness}"+(string.IsNullOrWhiteSpace(x.MappingId)?"":$" · eşleme {x.MappingId}"));var text=string.Join("\n",rows);productChannelSummary.Text=text;productChannelSummaryMirror.Text=text;}
