...
public partial class MainWindow : Window
{
 ...
 public MainWindow(string? directory, HttpClient? httpClient = null)
 {
  ...
  try{template=TemplateStore.Load(directory);templateEditor.DataContext=template;var saved=CredentialStore.Load(dataDirectory);if(saved!=null)SetCredentials(saved);}catch(Exception e){Log(Safe(e));}
 ...
 void BuildProducts()
 {
  ...
  bar.Children.Add(Button("Aktife al",()=>SetProductActive(true)));bar.Children.Add(Button("Pasife al",()=>SetProductActive(false)));bar.Children.Add(Button("Ürünü sil",DeleteSelectedProduct));bar.Children.Add(AsyncButton("Ürünleri XML'e aktar",ExportProductsToXmlAsync));productEditor.Children.Add(Heading("Ürün kartı"));productEditor.Children.Add(productPriceSummaryPanel);Field(productEditor,"Alış fiyatı","Cost").IsReadOnly=true;Field(productEditor,"Alış para birimi","CostCurrency").IsReadOnly=true;Field(productEditor,"Satış fiyatı","Price");Field(productEditor,"Satış para birimi","Currency").IsReadOnly=true;Field(productEditor,"KDV oranı (%)","VatRate");Field(productEditor,"Desi (kargo)","Desi");productEditor.Children.Add(Hint("XML güncellemesinde korunmasını istediğin alanı kilitle."));
 ...
 {
   source=s;BindSource();fxStatus.Text=RateDescription(s);calculationStatus.Text="Alış fiyatını girip hesaplamayı test edebilirsin.";itemPath.Text=s.ItemPath;decimalSeparator.SelectedItem=s.DecimalSeparator;previewMappingShapeFingerprint=""; _ = RefreshXmlSourceHealthAsync(s);
 ...
 async Task ScheduledAsync()
 {
  ...
  try{
   ...
   foreach(var job in dueJobs){try{var result=await Task.Run(()=>AutomationRunner.RunDue(store,new AutomationStore(dataDirectory),new SyncStore(dataDirectory),job.Id,DateTime.UtcNow));Log($"Otomasyon {job.Channel}/{job.Shop}: {result.Queued} iş kuyruğa alındı, {result.Errors.Count} hata.");}catch(Exception e){Log(Safe(e));}}
