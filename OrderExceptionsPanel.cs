...
public static class OrderExceptionsPanel
{
    public static FrameworkElement Create(string? directory, Action<string>? navigate = null)
    {
        ...
        foreach (var (header, path, width) in new[] { ("Öncelik", "Severity", 80d), ("Kanal", "Marketplace", 90d), ("Mağaza", "ShopId", 100d), ("Sipariş", "OrderId", 120d), ("Tür", "Type", 120d), ("Durum", "Status", 110d), ("Güncelleme", "UpdatedUtc", 150d), ("Açıklama", "Message", 350d) }) grid.Columns.Add(new DataGridTextColumn { Header = header, Binding = new Binding(path), Width = width });
        void RefreshGrid()
        {
        ...
        var bar = new WrapPanel(); bar.Children.Add(new TextBlock { Text = "Ara", Margin = new Thickness(4), VerticalAlignment = VerticalAlignment.Center }); bar.Children.Add(query); bar.Children.Add(state); bar.Children.Add(type); bar.Children.Add(age); bar.Children.Add(refresh); bar.Children.Add(reconcile); bar.Children.Add(openOrder); bar.Children.Add(openProduct); bar.Children.Add(previewButton); bar.Children.Add(approve); bar.Children.Add(reject); panel.Children.Add(bar); panel.Children.Add(grid); panel.Children.Add(detail); panel.Children.Add(status);
    ...
    static Button Button(string text, Action action) { var button = new Button { Content = text, Margin = new Thickness(3) }; button.Click += (_, _) => { try { action(); } catch (Exception error) { MessageBox.Show(MarketplaceConnectionStore.Redact(error.Message), "Sipariş istisnaları", MessageBoxButton.OK, MessageBoxImage.Warning); } }; return button; }
