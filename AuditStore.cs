...
public sealed class DiagnosticsService
{
    ...
    public DiagnosticsSnapshot Build()
    {
        ...
        checks.Add(new ProductDocumentStore(directory).Check()); // #911
...
{
    public static string Export(string outputPath, string? dataDirectory = null, bool overwrite = false)
    {
        dataDirectory ??= Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "MonoBridgeDesktop"); Directory.CreateDirectory(Path.GetDirectoryName(Path.GetFullPath(outputPath))!); var audit = new AuditStore(dataDirectory); var diagnostics = new DiagnosticsService(dataDirectory).Build(); var connectionMetadata = new MarketplaceConnectionStore(dataDirectory).List().Select(x => new { x.Id, x.Channel, x.ShopId, x.DisplayName, x.Enabled, x.Status, x.LastTestUtc, LastError = AuditStore.Sanitize(x.LastError) }).ToList(); var sourceMetadata = new Catalog.CatalogStore(dataDirectory).Sources().Select(x => new { x.Id, x.Name, Location = AuditStore.Sanitize(x.Location), x.Enabled, x.LastStatus, x.LastRunUtc }).ToList(); ExportFiles.Write(outputPath, overwrite, CancellationToken.None, temp => { using (var archive = ZipFile.Open(temp, ZipArchiveMode.Create)) { WriteJson(archive, "diagnostics.json", diagnostics); WriteJson(archive, "connections-metadata.json", connectionMetadata); WriteJson(archive, "xml-sources-metadata.json", sourceMetadata); WriteJson(archive, "audit.json", audit.List(AuditStore.RetentionLimit)); var latency = new LatencyStore(dataDirectory); WriteJson(archive, "ui-latency.json", new { Summary = latency.Summary(), Recent = latency.List(200) }); var logPath = Path.Combine(dataDirectory, "operations.log"); var entry = archive.CreateEntry("operations.log"); using (var writer = new StreamWriter(entry.Open())) { if (File.Exists(logPath)) foreach (var line in File.ReadLines(logPath).TakeLast(1000)) writer.WriteLine(AuditStore.Sanitize(line)); } var readme = archive.CreateEntry("README.txt"); using (var readmeWriter = new StreamWriter(readme.Open())) { readmeWriter.WriteLine("MonoBridge güvenli destek paketi"); readmeWriter.WriteLine("Credential/token/password değerleri pakete dahil edilmez; yerel DB dosyaları ve şifreli credential byte'ları paylaşılmaz."); } } }); return outputPath;
    }
