...
public sealed class SafetyAndStabilityTests
{
    ...
    public void SqliteConnectionPolicy_AppliesSharedSafetyPragmas()
    {
        ...
        try
        {
            using var connection = SqliteConnectionPolicy.Open(new SqliteConnectionStringBuilder { DataSource = Path.Combine(root, "policy.db") }.ToString());
    ...
    [TestMethod]
    public void FulfillmentCoreRemainsDeferredWithoutProviderRequest()
    {
        var decision = FulfillmentCoreDeferredGuard.Evaluate("o-1");
        Assert.AreEqual("DEFERRED_BY_USER", decision.Status);
        Assert.AreEqual("o-1", decision.OrderId);
        Assert.ThrowsException<InvalidOperationException>(() => FulfillmentCoreDeferredGuard.EnsureNoRequest(decision));
    }
    ...
    [TestMethod]
    public void EtsyLiveSafetyGateBlocksUnknownOrMismatchedCurrency()
    {
        Assert.AreEqual("READY", EtsyLiveSafetyGate.ValidateCurrency("USD", "usd").Status);
        Assert.AreEqual("BLOCKED", EtsyLiveSafetyGate.ValidateCurrency("USD", "EUR").Status);
        Assert.AreEqual("BLOCKED", EtsyLiveSafetyGate.ValidateCurrency("", "USD").Status);
    }
