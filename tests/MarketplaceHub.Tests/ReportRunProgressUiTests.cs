...
// #848 on the real reports panel: a run shows the three stage rows and ends with every row done and the file
// written; a failed export shows the export row failed, sanitized diagnostics, "Tanılamaya git" (which navigates)
// and "Yeniden dene", and the retry re-runs into the same file without asking again; on the setup itself a long
// run shows the query row indeterminate while the total is unknown, the cancel button stops it, the cancelled
// stage is marked, and retry is offered.
[TestClass]
        ...
        RunSta(() =>
        {
            try
            {
                ...
                try
                {
                    ...
                    WaitUntil(window, () => Status().StartsWith("Rapor yazılamadı", StringComparison.Ordinal), "the failed run");
                    Assert.AreEqual(Visibility.Visible, progress.Visibility); Assert.AreEqual(1, asked);
                    Assert.IsTrue(Descendants(setupHost).OfType<DockPanel>().Any(d => (string?)d.Tag == "report-run-stage-query") && Descendants(setupHost).OfType<DockPanel>().Any(d => (string?)d.Tag == "report-run-stage-export"), "Three stage rows.");
                    StringAssert.StartsWith(Descendants(Setup<DockPanel>("report-run-stage-query")).OfType<TextBlock>().First().Text, "✔ Sorgu");
                    StringAssert.StartsWith(Descendants(Setup<DockPanel>("report-run-stage-export")).OfType<TextBlock>().First().Text, "✖ Dışa aktarma");
                    ...
                    Assert.IsTrue(Setup<Button>("report-param-run").IsEnabled, "The run is available again after a failure.");

                    // Retry re-runs into the same file without asking; here the same directory blocks again, so the retry fails the same way and asks nothing.
                    Click(Setup<Button>("report-run-retry"));
                    WaitUntil(window, () => Status().StartsWith("Rapor yazılamadı", StringComparison.Ordinal), "the retried run");
                    Assert.AreEqual(1, asked, "A retry never asks for the file again.");

                    // A fresh run into a good file: every row done, the file written, retry and diagnostics gone, the card shows the run.
                    chosen = goodPath; Click(Setup<Button>("report-param-run"));
                    WaitUntil(window, () => Status().Contains("sipariş yazıldı"), "the good run");
                    Assert.AreEqual(2, asked); Assert.IsTrue(File.Exists(goodPath));
                    Assert.IsTrue(ReportRunProgressState.Stages.All(s => Descendants(Setup<DockPanel>("report-run-stage-" + s.Stage.ToString().ToLowerInvariant())).OfType<TextBlock>().First().Text.StartsWith("✔", StringComparison.Ordinal)));
                    ...
                    Assert.IsTrue(Descendants(setupHost).OfType<ProgressBar>().All(b => !b.IsIndeterminate && b.Value == 100));
                    ...
                    // A long run on the setup itself: unknown total -> indeterminate query bar; cancel stops it at its stage; retry appears.
                    var gate = new TaskCompletionSource<bool>(); CancellationToken seen = default; var retries = 0;
                    var slow = ReportParameterPanel.Build(new ReportParameterPanel.Context(ReportCatalog.Find("orders-csv")!, () => allowed, new UiPreferenceStore(root), navigated.Add, async (_, reporter, token, retry) =>
                    {
                        if (retry) retries++;
                        seen = token; reporter.Report(new(ReportRunStage.Query, ReportRunStageStatus.Running));
                        await Task.WhenAny(gate.Task, Task.Delay(Timeout.Infinite, token)).ConfigureAwait(true);
                        token.ThrowIfCancellationRequested();
                        reporter.Report(new(ReportRunStage.Query, ReportRunStageStatus.Done, 1, 1)); return "bitti";
                    }));
                    ...
                    var queryBar = Setup<ProgressBar>("report-run-bar-query");
                    Assert.IsTrue(queryBar.IsIndeterminate, "An unknown total is an indeterminate bar."); StringAssert.StartsWith(Setup<TextBlock>("report-run-headline").Text, "Sorgu sürüyor");
                    ...
                    Assert.AreEqual(1, retries); Assert.AreEqual(Visibility.Collapsed, Setup<Button>("report-run-retry").Visibility);
                }
