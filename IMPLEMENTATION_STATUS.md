...
M18 Navlungo yerel tracking geçmişi (2026-09-11, Issue #25): Navlungo paneli uygulama veri dizinine bağlandı; `NavlungoTracking` SQLite tablosu composite shipment upsert, sipariş filtresi ve maskeli hata alanı ile eklendi. Orders panelindeki yerel paket gözlemleri de aynı güvenli geçmişe yazılıyor. UI yalnızca yerel/manual kayıt gösterir; token, fulfillment, teklif, kargo oluşturma ve canlı tracking isteği yapılmaz (`LIVE_API_BLOCKED`). 290 Release testi geçti; `Windows-M18-Navlungo` self-contained yayın üretildi.
