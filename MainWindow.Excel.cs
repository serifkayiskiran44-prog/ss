...
public partial class MainWindow
{
    ...
    FrameworkElement BuildExcel()
    {
        ...
        var export = Button("Filtreli ürünleri dışa aktar", () => { ApplyProfileText(); var query = new TextBox { Width = 380, Text = "", ToolTip = "SKU/ürün/marka/kategori filtre metni" }; var dialog = new Window { Title = "Dışa aktarım filtresi", Width = 480, Height = 180, Owner = this, WindowStartupLocation = WindowStartupLocation.CenterOwner }; var ok = new Button { Content = "Excel'e aktar", Margin = Spacing.Control }; var box = new StackPanel { Margin = new Thickness(14) }; box.Children.Add(new TextBlock { Text = "Filtre (boş: tüm ürünler)" }); box.Children.Add(query); box.Children.Add(ok); dialog.Content = box; ok.Click += (_, _) => dialog.DialogResult = true; if (dialog.ShowDialog() != true) return; var products = store.Products().Where(p => string.IsNullOrWhiteSpace(query.Text) || $"{p.Sku} {p.Barcode} {p.Name} {p.Brand} {p.Category}".Contains(query.Text.Trim(), StringComparison.CurrentCultureIgnoreCase)).ToList(); var save = new SaveFileDialog { Filter = "Excel dosyası (*.xlsx)|*.xlsx", FileName = ExportFileNames.Build("urunler", "xlsx") }; if (save.ShowDialog(this) != true) return; var started = DateTime.UtcNow; CatalogExcel.Export(save.FileName, products, profile.VisibleFields); new ReportRunStore(dataDirectory).Record("products-xlsx", started, ReportRunState.Succeeded, products.Count); status.Text = $"{products.Count} ürün ve seçili alanlar dışa aktarıldı."; });
        ...
        var cancelRejectedExport = new Button { Content = "Dışa aktarımı iptal et", Margin = Spacing.Control, Visibility = Visibility.Collapsed };
        cancelRejectedExport.Click += (_, _) => rejectedExportCts?.Cancel();
        Button errors = null!; errors = Button("Reddedilen satırları dışa aktar", () => { if (preview is null || preview.Errors.Count == 0) { status.Text = "Dışa aktarılacak reddedilen satır yok."; return; } var dialog = new SaveFileDialog { Filter = "Excel dosyası (*.xlsx)|*.xlsx|CSV (*.csv)|*.csv", FileName = ExportFileNames.Build("reddedilen-satirlar", "xlsx") }; if (dialog.ShowDialog(this) != true) return; _ = ExportRejectedAsync(dialog.FileName, preview); });
        async Task ExportRejectedAsync(string path, ExcelPreview current)
        {
            ...
            try
            {
                var result = await Task.Run(() => path.EndsWith(".csv", StringComparison.OrdinalIgnoreCase) ? RejectedRowsExport.WriteCsv(path, RejectedRowsExport.Build(current.Errors), token, overwrite: true) : CatalogExcel.ExportErrors(path, current, token, overwrite: true), token);
