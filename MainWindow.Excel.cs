...
public partial class MainWindow
{
    FrameworkElement BuildExcel()
    {
        ...
        var status = Hint("");
        var sheetBox = new ComboBox { Width = 170, ToolTip = "Çalışma sayfası" };
        var headerRowBox = new TextBox { Text = "1", Width = 45, ToolTip = "Başlık satırı numarası (1'den başlar)" };
        ...
        void ApplyProfileText()
        {
            ...
            profile.SheetName = sheetBox.SelectedItem as string; profile.HeaderRow = int.TryParse(headerRowBox.Text, out var hr) && hr >= 1 ? hr : 1;
        ...
        void RenderPreview()
        {
            ...
            ApplyProfileText();
            var stale = CatalogExcel.IsProfileStale(profile, selectedPath);
            decisions = CatalogExcel.PreviewDecisions(store, selectedPath, profile); preview = new ExcelPreview(decisions.Rows.Where(x => x.Product is not null).Select(x => x.Product!).ToList(), decisions.Errors); grid.ItemsSource = decisions.Rows;
            var summary = $"{decisions.Rows.Count} satır önizlendi · {decisions.Rows.Count(x => x.Action == "CREATE")} yeni · {decisions.Rows.Count(x => x.Action == "UPDATE")} güncellenecek · {decisions.Rows.Count(x => x.Action == "SKIP")} aynı · {decisions.Rows.Count(x => x.Action == "ERROR")} hatalı.";
            status.Text = stale ? "UYARI: Bu dosyanın başlık satırı profil kaydedildiğinden beri değişmiş; kolon eşlemesini kontrol edin. " + summary : summary;
        }
        void LoadProfiles()
        {
        ...
        profileBox.SelectionChanged += (_, _) => { if (profileBox.SelectedItem is not ExcelImportProfile selected) return; profile = selected; profileName.Text = profile.Name; culture.Text = profile.CultureName; aliases.Text = string.Join(Environment.NewLine, profile.HeaderAliases.Select(x => $"{x.Key}={x.Value}")); defaults.Text = string.Join(Environment.NewLine, profile.Defaults.Select(x => $"{x.Key}={x.Value}")); visibleFields.Text = string.Join(",", profile.VisibleFields); headerRowBox.Text = profile.HeaderRow.ToString(); if (sheetBox.ItemsSource is not null && profile.SheetName is not null && sheetBox.Items.Contains(profile.SheetName)) sheetBox.SelectedItem = profile.SheetName; if (selectedPath is not null) RenderPreview(); };
        var saveProfile = Button("Profili kaydet", () => { ApplyProfileText(); if (selectedPath is not null) { try { profile.ExpectedHeaders = CatalogExcel.Headers(selectedPath, profile.SheetName, profile.HeaderRow).ToList(); } catch (IOException) { } catch (InvalidDataException) { } } profileStore.Save(profile); LoadProfiles(); status.Text = "Excel profili yerel olarak kaydedildi."; });
        var newProfile = Button("Yeni profil", () => { profile = new ExcelImportProfile { Name = "Yeni profil" }; profileName.Text = profile.Name; culture.Text = profile.CultureName; aliases.Clear(); defaults.Clear(); headerRowBox.Text = "1"; if (selectedPath is not null) RenderPreview(); });
        var choose = Button("Excel seç ve önizle", () =>
        {
            var dialog = new OpenFileDialog { Filter = "Excel dosyası (*.xlsx)|*.xlsx" }; if (dialog.ShowDialog(this) != true) return;
            selectedPath = dialog.FileName; manualMapping = null;
            var sheets = CatalogExcel.ListWorksheets(selectedPath); sheetBox.ItemsSource = sheets;
            sheetBox.SelectedItem = profile.SheetName is not null && sheets.Contains(profile.SheetName) ? profile.SheetName : sheets[0];
            RenderPreview();
        });
        sheetBox.SelectionChanged += (_, _) => { if (selectedPath is not null) RenderPreview(); };
        headerRowBox.TextChanged += (_, _) => { if (selectedPath is not null) RenderPreview(); };
        var map = Button("Kolonları elle eşle", () => { if (selectedPath is null) { status.Text = "Önce Excel dosyasını seçin."; return; } ApplyProfileText(); var headers = CatalogExcel.Headers(selectedPath, profile.SheetName, profile.HeaderRow); var result = ShowMappingDialog(headers, manualMapping); if (result is null) return; manualMapping = result; profile.ColumnMappings = result.Columns.ToDictionary(x => x.Key, x => headers[x.Value - 1], StringComparer.OrdinalIgnoreCase); RenderPreview(); });
        ...
        var exportXml = Button("Filtreli ürünleri XML'e aktar", () => { var products = store.Products(); var save = new SaveFileDialog { Filter = "XML dosyası (*.xml)|*.xml", FileName = "urunler.xml" }; if (save.ShowDialog(this) != true) return; CatalogXmlExport.Export(save.FileName, products); status.Text = $"{products.Count} ürün XML olarak (şema v{CatalogXmlExport.SchemaVersion}) dışa aktarıldı."; });
        ...
        var profileRow = new WrapPanel(); profileRow.Children.Add(new TextBlock { Text = "Profil", Margin = new Thickness(4), VerticalAlignment = VerticalAlignment.Center }); profileRow.Children.Add(profileBox); profileRow.Children.Add(new TextBlock { Text = "Ad", Margin = new Thickness(4), VerticalAlignment = VerticalAlignment.Center }); profileRow.Children.Add(profileName); profileRow.Children.Add(new TextBlock { Text = "Kültür", Margin = new Thickness(4), VerticalAlignment = VerticalAlignment.Center }); profileRow.Children.Add(culture); profileRow.Children.Add(new TextBlock { Text = "Sayfa", Margin = new Thickness(4), VerticalAlignment = VerticalAlignment.Center }); profileRow.Children.Add(sheetBox); profileRow.Children.Add(new TextBlock { Text = "Başlık satırı", Margin = new Thickness(4), VerticalAlignment = VerticalAlignment.Center }); profileRow.Children.Add(headerRowBox); profileRow.Children.Add(saveProfile); profileRow.Children.Add(newProfile); panel.Children.Add(profileRow);
        ...
        var bar = new WrapPanel(); bar.Children.Add(export); bar.Children.Add(exportXml); bar.Children.Add(choose); bar.Children.Add(map); bar.Children.Add(errors); bar.Children.Add(apply); bar.Children.Add(rollback); panel.Children.Add(bar); panel.Children.Add(grid); panel.Children.Add(status); LoadProfiles(); return Scroll(panel);
    }
    ...
    static ExcelColumnMapping? ShowMappingDialog(IReadOnlyList<string> headers, ExcelColumnMapping? existing)
    {
        ...
        var window = new Window { Title = "Excel kolon eşleme", Width = 520, Height = 700, WindowStartupLocation = WindowStartupLocation.CenterOwner, ResizeMode = ResizeMode.CanResize, Owner = Application.Current?.Windows.OfType<MainWindow>().FirstOrDefault() };
        ...
        var result = new ExcelColumnMapping?[] { null }; var ok = new Button { Content = "Eşlemeyi kullan", Margin = new Thickness(0, 12, 0, 0) }; ok.Click += (_, _) => { var map = new Dictionary<string, int>(); foreach (var field in fields) { var value = combos[field.Item1].SelectedItem?.ToString(); if (value is null or "(eşlenmemiş)") continue; var index = headers.IndexOf(value); if (index >= 0) map[field.Item1] = index + 1; } result[0] = new ExcelColumnMapping(map); window.DialogResult = true; }; content.Children.Add(ok); window.Content = new ScrollViewer { Content = content }; window.ShowDialog(); return result[0];
    }
}
