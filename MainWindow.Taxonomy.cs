...
public partial class MainWindow
{
    FrameworkElement BuildTaxonomy()
    {
        ...
        var attributeRules = new CategoryAttributeRuleStore(dataDirectory); var ruleGrid = new DataGrid { AutoGenerateColumns = true, IsReadOnly = true, Height = 200, EnableRowVirtualization = true }; // #915
        var reviewQueue = new CategoryReviewQueue(dataDirectory); var reviewGrid = new DataGrid { AutoGenerateColumns = true, IsReadOnly = true, Height = 220, EnableRowVirtualization = true, SelectionMode = DataGridSelectionMode.Extended }; // #913
        ...
        void Refresh()
        {
            ...
            localGrid.ItemsSource = local; var views = taxonomy.MappingViews(selected, market, shopId, search.Text.Trim()); mappingGrid.ItemsSource = views; historyGrid.ItemsSource = taxonomy.History(selected, market, shopId); aliasGrid.ItemsSource = aliases.List(); /* #912 */ reviewGrid.ItemsSource = reviewQueue.List(CategoryReviewItem.Pending); /* #913 */ ruleGrid.ItemsSource = attributeRules.List(); /* #915 */
        ...
        var approveReview = Button("Seçili kuyruk değerlerini seçili kategoriye onayla", () => { if (localGrid.SelectedItem is not TaxonomyEntry entry) throw new InvalidOperationException("Önce yerel listeden kategori seçin."); var items = reviewGrid.SelectedItems.OfType<CategoryReviewItem>().ToList(); if (items.Count == 0) throw new InvalidOperationException("Önce inceleme kuyruğundan değer seçin."); var preview = string.Join("\n", items.Take(20).Select(i => $"• {i.Value} ({i.Count} satır, kaynak {i.SourceId[..Math.Min(8, i.SourceId.Length)]})")) + (items.Count > 20 ? $"\n… ve {items.Count - 20} değer daha" : ""); if (!DialogShell.Confirm(this, "Toplu onay", $"{items.Count} değer '{entry.Name}' kategorisinin onaylı takma adı olacak; sonraki içe aktarmalar bunları uygular.\n{preview}", "Onayla", destructive: false)) return; var done = reviewQueue.ApproveBulk(items, entry.Id, true, DateTime.UtcNow); Refresh(); status.Text = $"{done} değer onaylandı ve sözlüğe yazıldı."; });
        var rejectReview = Button("Seçili kuyruk değerlerini reddet", () => { var items = reviewGrid.SelectedItems.OfType<CategoryReviewItem>().ToList(); if (items.Count == 0) throw new InvalidOperationException("Önce inceleme kuyruğundan değer seçin."); if (!DialogShell.Confirm(this, "Toplu ret", $"{items.Count} değer reddedilecek; bir sonraki görülmede yeniden kuyruğa girer.", "Reddet")) return; var done = reviewQueue.RejectBulk(items, true, DateTime.UtcNow); Refresh(); status.Text = $"{done} değer reddedildi."; });
        var reviewRow = new WrapPanel(); reviewRow.Children.Add(approveReview); reviewRow.Children.Add(rejectReview); panel.Children.Add(reviewRow);
        ...
        var ruleAttribute = new TextBox { Width = 220 }; var ruleRequired = new CheckBox { Content = "Zorunlu", IsChecked = true, VerticalAlignment = VerticalAlignment.Center, Margin = Spacing.Inline };
        var addRule = Button("Özellik kuralı ekle", () => { if (localGrid.SelectedItem is not TaxonomyEntry entry) throw new InvalidOperationException("Önce yerel listeden kategori seçin."); var rule = attributeRules.Save(entry.Id, ruleAttribute.Text, ruleRequired.IsChecked == true); ruleAttribute.Clear(); Refresh(); status.Text = $"Özellik kuralı kaydedildi: {rule.CategoryName} · {rule.Attribute} ({(rule.Required ? "zorunlu" : "isteğe bağlı")})."; });
        var removeRule = Button("Seçili özellik kuralını kaldır", () => { if (ruleGrid.SelectedItem is not CategoryAttributeRule rule) throw new InvalidOperationException("Önce kural listesinden seçim yapın."); attributeRules.Remove(rule.CategoryId, rule.Attribute); Refresh(); });
        var ruleRow = new WrapPanel(); Label(ruleRow, "Özellik adı (seçili kategori için)", ruleAttribute); ruleRow.Children.Add(ruleRequired); ruleRow.Children.Add(addRule); ruleRow.Children.Add(removeRule); panel.Children.Add(ruleRow);
        ...
        var tabs = new TabControl(); tabs.Items.Add(new TabItem { Header = "Yerel sözlük", Content = localGrid }); tabs.Items.Add(new TabItem { Header = "Kanal / mağaza eşleme", Content = mappingGrid }); tabs.Items.Add(new TabItem { Header = "Eşleme geçmişi", Content = historyGrid }); tabs.Items.Add(new TabItem { Header = "Takma adlar", Content = aliasGrid }); /* #912 */ tabs.Items.Add(new TabItem { Header = "İnceleme kuyruğu", Content = reviewGrid }); /* #913 */ tabs.Items.Add(new TabItem { Header = "Özellik kuralları", Content = ruleGrid }); /* #915 */ panel.Children.Add(tabs); panel.Children.Add(status); Refresh(); return Scroll(panel);
    }
}
