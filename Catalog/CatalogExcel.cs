...
public sealed record ExcelColumnMapping(IReadOnlyDictionary<string, int> Columns)
{
    public static ExcelColumnMapping FromHeaders(IXLRow header, IReadOnlyDictionary<string, string>? aliases = null)
    {
        ...
        {
            ["Id"] = ["ürün id", "urun id", "product id", "id"], ["Sku"] = ["sku", "stockcode", "stokkodu", "code"], ["Barcode"] = ["barkod", "barcode", "ean"], ["Name"] = ["ürün", "urun", "ürün adı", "urun adi", "name", "title"], ["Brand"] = ["marka", "brand"], ["Category"] = ["kategori", "category"], ["Description"] = ["açıklama", "aciklama", "description"], ["Cost"] = ["alış", "alis", "maliyet", "cost"], ["Price"] = ["satış", "satis", "fiyat", "price"], ["Currency"] = ["döviz", "doviz", "currency"], ["VatRate"] = ["kdv", "vat", "vat rate", "kdv oranı"], ["Stock"] = ["stok", "stock", "qty", "quantity"], ["Active"] = ["aktif", "active", "enabled"], ["Gtin"] = ["gtin", "gtin13", "gtin14"], ["ImageUrls"] = ["görseller", "gorseller", "images", "imageurls"]
        };
...
public static class CatalogExcel
{
    ...
    [
        ("Id", "Ürün ID", ExportCellKind.Text, p => p.Id), ("Sku", "SKU", ExportCellKind.Text, p => p.Sku), ("Barcode", "Barkod", ExportCellKind.Text, p => p.Barcode), ("Name", "Ürün", ExportCellKind.Text, p => p.Name),
    ...
    {
        var preview = Preview(path, profile); var existing = store.Products(); var byId = existing.ToDictionary(x => x.Id, StringComparer.OrdinalIgnoreCase); var bySku = existing.Where(x => x.Sku.Length > 0).GroupBy(x => x.Sku, StringComparer.OrdinalIgnoreCase).ToDictionary(x => x.Key, x => x.Single(), StringComparer.OrdinalIgnoreCase); var byBarcode = existing.Where(x => x.Barcode.Length > 0).GroupBy(x => x.Barcode, StringComparer.OrdinalIgnoreCase).ToDictionary(x => x.Key, x => x.Single(), StringComparer.OrdinalIgnoreCase); var decisions = new List<ExcelPreviewDecision>(); var row = 2; foreach (var product in preview.Rows) { var match = product.Id.Length > 0 ? byId.GetValueOrDefault(product.Id) : bySku.GetValueOrDefault(product.Sku) ?? (product.Barcode.Length == 0 ? null : byBarcode.GetValueOrDefault(product.Barcode)); var action = match is null ? "CREATE" : Equivalent(match, product) ? "SKIP" : "UPDATE"; decisions.Add(new(row++, action, product)); } foreach (var error in preview.Errors) decisions.Add(new(row++, "ERROR", null, error)); return new(decisions, preview.Errors);
    }
    static bool Equivalent(CatalogProduct left, CatalogProduct right) => (right.Id.Length == 0 || left.Id.Equals(right.Id,StringComparison.OrdinalIgnoreCase)) && left.Sku.Equals(right.Sku, StringComparison.OrdinalIgnoreCase) && left.Barcode.Equals(right.Barcode, StringComparison.OrdinalIgnoreCase) && left.Name == right.Name && left.Brand == right.Brand && left.Category == right.Category && left.Description == right.Description && left.Cost == right.Cost && left.Price == right.Price && left.Currency.Equals(right.Currency, StringComparison.OrdinalIgnoreCase) && left.VatRate == right.VatRate && left.Stock == right.Stock && left.Active == right.Active && left.Gtin == right.Gtin;
    ...
    public static CatalogUndoReceipt ApplyWithUndo(CatalogStore store, XmlSource source, ExcelPreview preview, IReadOnlyCollection<int> selectedRows, IReadOnlyCollection<string>? selectedFields = null) { if (selectedRows.Count == 0) throw new InvalidOperationException("Uygulamak için en az bir satır seçin."); ValidatePreview(preview); var rows = preview.Rows.Where((_, index) => selectedRows.Contains(index)).ToList(); if (rows.Count != selectedRows.Count) throw new InvalidOperationException("Seçili satır numarası önizleme dışında."); return store.ApplyExcelUpdatesWithUndo(rows, selectedFields); }
    static void ValidatePreview(ExcelPreview preview) { if (preview.Errors.Count > 0) throw new InvalidOperationException("Hatalı Excel satırları düzeltilmeden katalog güncellenemez."); foreach (var row in preview.Rows) if (!TrMarketplaceHubDesktop.LocaleSettings.SupportedCurrencies.Contains(row.Currency.Trim())) throw new InvalidOperationException($"Excel döviz değeri desteklenmiyor: {row.Currency}."); }
