...
public sealed class AutomationStore
{
    ...
    public bool ReleaseLease(string id, string leaseToken) { if (string.IsNullOrWhiteSpace(leaseToken)) return false; using var c = Open(); EnsureColumn(c, "LeaseToken", "TEXT NULL"); using var cmd = c.CreateCommand(); cmd.CommandText = "UPDATE AutomationJobs SET LockedUntilUtc=NULL,LeaseToken=NULL WHERE Id=$id AND LeaseToken=$token"; cmd.Parameters.AddWithValue("$id", id); cmd.Parameters.AddWithValue("$token", leaseToken); return cmd.ExecuteNonQuery() == 1; }
