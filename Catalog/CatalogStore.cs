...
public partial class CatalogStore
{
 ...
 static void Valid(CatalogProduct p){if(p.Mpn==null||p.Mpn.Length>128||p.InvoiceName==null||p.InvoiceName.Length>300||p.Subtitle==null||p.Subtitle.Length>300||p.Shelf==null||p.Shelf.Length>100)throw new InvalidOperationException("MPN en fazla 128, fatura adı/alt başlık 300, raf 100 karakter olabilir.");if(string.IsNullOrWhiteSpace(p.Name)||(string.IsNullOrWhiteSpace(p.Sku)&&string.IsNullOrWhiteSpace(p.Barcode))||p.Price<0||p.Cost<0||p.Stock<0||p.VatRate<0||p.VatRate>100)throw new InvalidOperationException("Ürün adı, kimliği, fiyatı, stoku veya KDV oranı geçersiz.");}
