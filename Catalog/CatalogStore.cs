...
public partial class CatalogStore
{
 ...
 {
  XmlCatalog.ValidateSource(source);using var c=Open();using var tx=c.BeginTransaction();var products=ReadProductsSafe(c,tx).Healthy;int added=0,updated=0,unchanged=0;var skus=new HashSet<string>(StringComparer.OrdinalIgnoreCase);var bars=new HashSet<string>(StringComparer.OrdinalIgnoreCase);var touched=new HashSet<string>();var skuIndex=new Dictionary<string,List<CatalogProduct>>(StringComparer.OrdinalIgnoreCase);var barcodeIndex=new Dictionary<string,List<CatalogProduct>>(StringComparer.OrdinalIgnoreCase);foreach(var p in products.Where(p=>p.SourceId==source.Id)){Index(skuIndex,p.Sku,p);Index(barcodeIndex,p.Barcode,p);}
