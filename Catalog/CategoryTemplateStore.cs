...
public sealed class CategoryTemplateStore
{
    ...
    public CategoryFieldTemplate Save(CategoryFieldTemplate template)
    {
        ...
        using var find = c.CreateCommand(); find.Transaction = tx; find.CommandText = "SELECT Id,Version FROM CategoryFieldTemplates WHERE CategoryId=$cat AND Channel=$channel AND ShopId=$shop";
        ...
        using (var r = find.ExecuteReader()) if (r.Read()) { existingId = r.GetString(0); nextVersion = r.GetInt32(1) + 1; }
    ...
    public IReadOnlyList<CategoryFieldTemplate> List()
    {
        ...
        using var r = cmd.ExecuteReader(); var result = new List<CategoryFieldTemplate>(); while (r.Read()) result.Add(Read(r)); return result;
    }

    static CategoryFieldTemplate Read(SqliteDataReader r) => new()
    {
        Id = r.GetString(0), CategoryId = r.GetString(1), Channel = r.GetString(2), ShopId = r.GetString(3), Version = r.GetInt32(4),
        Brand = r.IsDBNull(5) ? null : r.GetString(5), Currency = r.IsDBNull(6) ? null : r.GetString(6), VatRate = r.IsDBNull(7) ? null : r.GetDecimal(7),
        UpdatedUtc = DateTime.TryParse(r.GetString(8), CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var d) ? d : DateTime.MinValue,
    };
