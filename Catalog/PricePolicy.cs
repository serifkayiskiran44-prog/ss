...
public sealed record PricePreview(string Sku, decimal Price, string Currency, decimal FormulaPriceTry, decimal CostTry, string CostOrigin = ""); // #922: the net margin names where its cost came from
public partial class CatalogStore {
 ...
 public PricePreview PreviewPrice(string ch,string shop,string id){
  ...
  return new(product.Sku,salePrice,saleCurrency,formulaPriceTry,product.Cost,CostProvenance.Describe(product,SourceById,DateTime.UtcNow)); /* #922 */
 }
}
