...
public sealed partial class TrendyolWorkspacePanel
{
    ...
    void MatchProducts()
    {
        ...
        barcodeDrafts.Clear();barcodeDraftDirty=false;matchRows=selected.Select(row=>{var profile=matchSnapshot.Profiles.SingleOrDefault(p=>p.ProductId==row.Id);var current=profile?.IntegrationCode??"";var review=new TrendyolMatchReviewRow(row.Product,TrendyolWorkspaceStore.Hash(JsonSerializer.Serialize(row.Product)),current,null,"");var intended=profile?.ListingBarcode??"";return EvaluateBarcode(review,intended.Length>0?intended:row.Barcode.Length>0?row.Barcode:matchSnapshot.Products.Any(p=>p.Barcode==current)?current:"");}).ToList();
