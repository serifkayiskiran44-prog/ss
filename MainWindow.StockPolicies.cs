...
public partial class MainWindow {
 FrameworkElement BuildStockPolicies(){
  ...
  panel.Children.Add(Button("Kayıtlı ayarla stok önizle",()=>{if(product.SelectedItem is not CatalogProduct p)throw new InvalidOperationException("Ürün listesini yenileyip ürün seçin.");var projection=store.ProjectStock(channel.Text,shop.Text,p.Id,DateTime.UtcNow);result.Text=$"{p.Sku} → gösterilebilir stok: {projection.Available} · kaynak stoku: {projection.State} · {projection.Words}. Kaydedilmemiş form değişiklikleri kullanılmaz."; /* #932 */}));panel.Children.Add(result);return Scroll(panel);
 }
}
