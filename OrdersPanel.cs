...
public static class OrdersPanel
{
 ...
 {
  var store=new OrdersStore(directory);var catalog=new CatalogStore(directory);var navlungoTracking=new NavlungoTrackingStore(directory);var root=new DockPanel{Margin=new Thickness(12)};var top=new StackPanel();DockPanel.SetDock(top,Dock.Top);root.Children.Add(top);
  ...
  void Edit(OrderSnapshot order,bool isNew=false)
  {
   ...
   var save=Button(detail,"Yerel kaydı / gözlemleri kaydet");save.Click+=(_,_)=>{try{Capture();captureShipment();if(isNew&&store.ReadAll().Any(x=>x.Marketplace==o.Marketplace&&x.ShopId==o.ShopId&&x.OrderId==o.OrderId))throw new ArgumentException("Bu sipariş zaten kayıtlı; listeden açarak düzenleyin.");store.SaveManual(o);foreach(var shipment in o.Shipments.Where(x=>!string.IsNullOrWhiteSpace(x.Id)&&!string.IsNullOrWhiteSpace(x.TrackingNumber)))navlungoTracking.Save(new("",o.Marketplace,o.ShopId,o.OrderId,shipment.Id,shipment.TrackingNumber,shipment.Carrier,shipment.State,DateTime.UtcNow,shipment.Source,""));Load();var saved=all.First(x=>x.Marketplace==o.Marketplace&&x.ShopId==o.ShopId&&x.OrderId==o.OrderId);Edit(saved);status.Text="Yerel kayıt kaydedildi. Tracking geçmişi yerel olarak saklandı; pazaryerine veya taşıyıcıya gönderim yapılmadı.";}catch(ArgumentException ex){status.Text=ex.Message;}catch{status.Text="Yerel kayıt kaydedilemedi; disk erişimini kontrol edin.";}};
  ...
  Load();detail.Children.Add(Text("Ayrıntıları ve paket geçmişini görmek için listeden bir sipariş seçin. İlk kaydı eklemek için + Yerel sipariş düğmesini kullanın."));return root;
 }
