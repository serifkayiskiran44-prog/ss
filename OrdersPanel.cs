...
public static class OrdersPanel
{
 public static FrameworkElement Create(string? directory=null,Func<Task<EtsyCredentials>>? authorize=null,Action? catalogChanged=null,Action<Func<string,string,string,bool>>? exposeReveal=null)
 {
  ...
  // #835: urgency from recorded states -- a band filter and a deterministic sort (score, then key).
  var urgencyFilter=new ComboBox{Width=150,ToolTip="Aciliyet bandı",ItemsSource=new[]{"Aciliyet: Tümü"}.Concat(OrderUrgencyScorer.BandOptions.Select(b=>b.Label)).ToArray(),SelectedIndex=0};System.Windows.Automation.AutomationProperties.SetName(urgencyFilter,"Aciliyet bandı");bar.Children.Add(urgencyFilter);
  var urgencySort=new CheckBox{Content="Aciliyete göre sırala",IsChecked=true,VerticalAlignment=VerticalAlignment.Center,Margin=new Thickness(6,0,0,0)};System.Windows.Automation.AutomationProperties.SetName(urgencySort,"Aciliyete göre sırala");bar.Children.Add(urgencySort);
  ...
  var detailWidth=OrderWorkspaceLayout.Deserialize(viewStore.Get(OrderWorkspaceLayout.SplitPreferenceKey));var splitter=new GridSplitter{Width=6,HorizontalAlignment=HorizontalAlignment.Stretch,VerticalAlignment=VerticalAlignment.Stretch,Background=SystemColors.ControlLightBrush,Focusable=true,KeyboardIncrement=16,ToolTip="Liste ile ayrıntı arasındaki bölmeyi sürükle (ok tuşları da çalışır)"};System.Windows.Automation.AutomationProperties.SetName(splitter,"Liste ve ayrıntı bölmesi");
  var workspaceMode=OrderWorkspaceMode.SideBySide;
  ...
  foreach(var (label,path,width) in new[]{("Pazaryeri","Marketplace",90),("Mağaza","ShopId",100),("Sipariş","OrderId",110),("Sipariş durumu (kaynak)","RawStatus",150),("Ödeme","PaymentStatus",110),("Stok kararı","StockDecisionLabel",130),("Kargo","DeliveryLabel",210),("Teslimat SLA","SlaLabel",120),("Taşıyıcı","Carriers",110),("Takip no","TrackingNumbers",130),("Toplam","TotalLabel",100),("Kaynak","Source",120),("Son başarılı alım","SyncLabel",140),("Aciliyet","UrgencyLabel",190)})grid.Columns.Add(new DataGridTextColumn{Header=label,Binding=new Binding(path),MinWidth=width,Width=width});
  static string ColumnKey(DataGridColumn c)=>c is DataGridBoundColumn{Binding:Binding b}?b.Path.Path:c.Header?.ToString()??"";
  var applyingLayout=false;
  IReadOnlyList<DataGridColumnLayout> CurrentColumns()=>grid.Columns.Select(c=>new DataGridColumnLayout(ColumnKey(c),c.DisplayIndex,c.Width.IsAbsolute?c.Width.Value:c.ActualWidth,c.Visibility==Visibility.Visible)).ToList();
  void ApplyLayout(DataGridLayoutState? state){if(state==null)return;applyingLayout=true;try{var byKey=grid.Columns.GroupBy(ColumnKey,StringComparer.Ordinal).ToDictionary(g=>g.Key,g=>g.First(),StringComparer.Ordinal);var order=DataGridLayoutCodec.ResolveOrder(grid.Columns.Select(ColumnKey).ToList(),state.Columns);for(var i=0;i<order.Count;i++)if(byKey.TryGetValue(order[i],out var column)&&column.DisplayIndex!=i)column.DisplayIndex=i;foreach(var saved in state.Columns){if(!byKey.TryGetValue(saved.Key,out var column))continue;if(saved.Width>0)column.Width=new DataGridLength(saved.Width);column.Visibility=saved.Visible?Visibility.Visible:Visibility.Collapsed;}
   // A PII column (none today) never shows under a preset, whatever the saved state says.
   if(presetController.Current!=OrderColumnPresets.CustomKey)foreach(var column in grid.Columns)if(OrderColumnPresets.IsPii(ColumnKey(column)))column.Visibility=Visibility.Collapsed;}finally{applyingLayout=false;}}
  void SyncPresetCombo(){applyingLayout=true;try{columnPreset.SelectedIndex=OrderColumnPresets.Options.ToList().FindIndex(o=>o.Key==presetController.Current);}finally{applyingLayout=false;}}
  void UserEditedLayout(){if(applyingLayout||grid.Columns.Count==0)return;try{presetController.UserEdited(new DataGridLayoutState(DataGridLayoutCodec.CurrentVersion,CurrentColumns(),"",false));}catch(Exception e){status.Text="Kolon düzeni kaydedilemedi: "+MarketplaceConnectionStore.Redact(e.Message);}SyncPresetCombo();}
  columnPreset.SelectionChanged+=(_,_)=>{if(applyingLayout||columnPreset.SelectedIndex<0)return;var key=OrderColumnPresets.Options[columnPreset.SelectedIndex].Key;ApplyLayout(presetController.Select(key,grid.Columns.Select(ColumnKey).ToList(),CurrentColumns()));};
  ApplyLayout(presetController.Initial(grid.Columns.Select(ColumnKey).ToList(),CurrentColumns()));SyncPresetCombo();
  grid.ColumnDisplayIndexChanged+=(_,_)=>UserEditedLayout();grid.ColumnReordered+=(_,_)=>UserEditedLayout();
  var widthDescriptor=System.ComponentModel.DependencyPropertyDescriptor.FromProperty(DataGridColumn.WidthProperty,typeof(DataGridColumn));var visibilityDescriptor=System.ComponentModel.DependencyPropertyDescriptor.FromProperty(DataGridColumn.VisibilityProperty,typeof(DataGridColumn));foreach(var column in grid.Columns){widthDescriptor.AddValueChanged(column,(_,_)=>UserEditedLayout());visibilityDescriptor.AddValueChanged(column,(_,_)=>UserEditedLayout());}
  var detail=new StackPanel{Margin=new Thickness(12,0,0,0)};var scroll=new ScrollViewer{Content=detail,VerticalScrollBarVisibility=ScrollBarVisibility.Auto};layout.Children.Add(splitter);layout.Children.Add(scroll);
  void ApplyWorkspaceMode(double width){var mode=OrderWorkspaceLayout.Mode(width);if(mode==workspaceMode&&layout.ColumnDefinitions.Count+layout.RowDefinitions.Count>0)return;workspaceMode=mode;layout.ColumnDefinitions.Clear();layout.RowDefinitions.Clear();
   if(mode==OrderWorkspaceMode.SideBySide){layout.ColumnDefinitions.Add(new(){Width=new GridLength(1,GridUnitType.Star),MinWidth=OrderWorkspaceLayout.MinListWidth});layout.ColumnDefinitions.Add(new(){Width=GridLength.Auto});layout.ColumnDefinitions.Add(new(){Width=new GridLength(OrderWorkspaceLayout.ClampDetailWidth(detailWidth,width)),MinWidth=OrderWorkspaceLayout.MinDetailWidth});splitter.Width=6;splitter.Height=double.NaN;splitter.ResizeDirection=GridResizeDirection.Columns;Grid.SetRow(grid,0);Grid.SetRow(splitter,0);Grid.SetRow(scroll,0);Grid.SetColumn(grid,0);Grid.SetColumn(splitter,1);Grid.SetColumn(scroll,2);}
   else{layout.RowDefinitions.Add(new(){Height=new GridLength(1,GridUnitType.Star)});layout.RowDefinitions.Add(new(){Height=GridLength.Auto});layout.RowDefinitions.Add(new(){Height=new GridLength(OrderWorkspaceLayout.StackedDetailShare,GridUnitType.Star)});splitter.Height=6;splitter.Width=double.NaN;splitter.ResizeDirection=GridResizeDirection.Rows;Grid.SetColumn(grid,0);Grid.SetColumn(splitter,0);Grid.SetColumn(scroll,0);Grid.SetRow(grid,0);Grid.SetRow(splitter,1);Grid.SetRow(scroll,2);}}
  ApplyWorkspaceMode(0);layout.SizeChanged+=(_,e)=>ApplyWorkspaceMode(e.NewSize.Width);
  splitter.DragCompleted+=(_,_)=>{if(workspaceMode!=OrderWorkspaceMode.SideBySide||layout.ColumnDefinitions.Count<3)return;var column=layout.ColumnDefinitions[2];detailWidth=OrderWorkspaceLayout.ClampDetailWidth(column.ActualWidth>0?column.ActualWidth:column.Width.Value,layout.ActualWidth);try{viewStore.Set(OrderWorkspaceLayout.SplitPreferenceKey,OrderWorkspaceLayout.Serialize(detailWidth));}catch(Exception){}};
  List<OrderSnapshot> all=[];OrderSnapshot? editing=null;string? selectionKey=null;CancellationTokenSource? running=null;
  ...
  void Filter(){string q=search.Text.Trim();var selectedMarketplace=marketplaceFilter.SelectedItem?.ToString()??"Tümü";var selectedShop=shopFilter.SelectedItem?.ToString()??"Tümü";var selectedStock=stockFilter.SelectedItem?.ToString()??"Tümü";var selectedBand=urgencyFilter.SelectedIndex<=0?null:OrderUrgencyScorer.BandOptions[urgencyFilter.SelectedIndex-1].Key;var filtered=all.Where(o=>(selectedBand==null||OrderUrgencyScorer.Band(o.UrgencyScore)==selectedBand||(selectedBand==OrderUrgencyScorer.CancelledBand&&o.UrgencyLabel.StartsWith(OrderUrgencyScorer.CancelledBand,StringComparison.Ordinal)))&&(q.Length==0||$"{o.Marketplace} {o.ShopId} {o.OrderId} {o.TrackingNumbers} {string.Join(' ',o.Items.Select(i=>i.Title+" "+i.Sku))}".Contains(q,StringComparison.CurrentCultureIgnoreCase))&&(filter.SelectedIndex<=0||o.Shipments.Any(s=>s.State==OrdersRules.States[filter.SelectedIndex-1])||(filter.SelectedIndex==1&&o.Shipments.Count==0))&&(selectedMarketplace=="Tümü"||o.Marketplace==selectedMarketplace)&&(selectedShop=="Tümü"||o.ShopId==selectedShop)&&(selectedStock=="Tümü"||(selectedStock=="Stok düşüldü"&&o.StockDecisionLabel=="Stok düşüldü")||(selectedStock=="Stok bekliyor"&&o.StockDecisionLabel!="Stok düşüldü")));var keepKey=grid.SelectedItem is OrderSnapshot kept?OrderWorkspaceLayout.SelectionKey(kept):selectionKey;grid.ItemsSource=urgencySort.IsChecked==true?OrderUrgencyScorer.Sort(filtered,o=>o.UrgencyScore,o=>$"{o.Marketplace}/{o.ShopId}/{o.OrderId}"):filtered.ToList();
   // #836: the same order stays selected across a reload (fresh instances) or a filter change; when it is no longer listed the detail closes -- a detail never shows an order the list does not.
   var restored=OrderWorkspaceLayout.Restore((IEnumerable<OrderSnapshot>)grid.ItemsSource,keepKey);if(restored!=null){if(!ReferenceEquals(grid.SelectedItem,restored))grid.SelectedItem=restored;}else if(keepKey!=null){selectionKey=null;editing=null;detail.Children.Clear();detail.Children.Add(Text("Seçili sipariş bu listede değil; ayrıntı kapatıldı. Listeden bir sipariş seçin."));}}
  void Load(){all=store.ReadAll();foreach(var order in all)order.StockDecisionLabel=catalog.GetOrderStockStatus(order.Marketplace,order.ShopId,order.OrderId)==null?"Stok bekliyor":"Stok düşüldü";
   var knownSkus=catalog.Products().Where(p=>!string.IsNullOrWhiteSpace(p.Sku)).Select(p=>p.Sku.Trim()).ToHashSet(StringComparer.OrdinalIgnoreCase);Dictionary<(string,string,string),int> pending;try{pending=new OrderExceptionStore(directory).List(status:"Pending").GroupBy(x=>(x.Marketplace,x.ShopId,x.OrderId)).ToDictionary(g=>g.Key,g=>g.Count());}catch(Exception){pending=new();}
