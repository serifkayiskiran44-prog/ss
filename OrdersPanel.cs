...
public static class OrdersPanel
{
 public static FrameworkElement Create(string? directory=null,Func<Task<EtsyCredentials>>? authorize=null,Action? catalogChanged=null)
 {
  ...
  var inventoryLocations=Button(bar,"Fiziksel konumları yönet");inventoryLocations.Name="InventoryLocationsButton";
 ...
 static void InventoryLocationsDialog(string? directory)
 {
  var model=new InventoryLocationManagementModel(directory);var window=new Window{Title="Fiziksel envanter konumları",Width=520,Height=430,WindowStartupLocation=WindowStartupLocation.CenterOwner};var panel=new StackPanel{Margin=new Thickness(14)};window.Content=panel;
  panel.Children.Add(Text("Fiziksel mağaza / depo adı"));var name=new TextBox{Name="InventoryLocationName"};panel.Children.Add(name);var status=Text("");panel.Children.Add(status);var list=new ListBox{Name="InventoryLocationList",DisplayMemberPath="Name",Height=220};panel.Children.Add(list);
  void Refresh()=>list.ItemsSource=model.Locations.Where(x=>x.Kind==InventoryLocationKind.PhysicalStore).ToArray();
  var create=Button(panel,"Fiziksel konum oluştur");create.Name="InventoryLocationCreateButton";create.Click+=(_,_)=>{try{var created=model.CreatePhysicalStore(name.Text);status.Text=$"{created.Name} oluşturuldu. Ürün Kaynakları ve Stok ekranındaki değişmez transfer önizlemesiyle stok aktarabilirsiniz.";name.Clear();Refresh();}catch(Exception error){status.Text=MarketplaceConnectionStore.Redact(error.Message);}};
