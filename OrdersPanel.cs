...
public static class OrdersPanel
{
 public static FrameworkElement Create(string? directory=null,Func<Task<EtsyCredentials>>? authorize=null,Action? catalogChanged=null,Action<Func<string,string,string,bool>>? exposeReveal=null)
 {
  ...
  var urgencyFilter=new ComboBox{Width=150,ToolTip="Aciliyet bandı",ItemsSource=new[]{"Aciliyet: Tümü"}.Concat(OrderUrgencyScorer.Bands.Select(b=>char.ToUpper(b[0],System.Globalization.CultureInfo.CurrentCulture)+b[1..])).ToArray(),SelectedIndex=0};System.Windows.Automation.AutomationProperties.SetName(urgencyFilter,"Aciliyet bandı");bar.Children.Add(urgencyFilter);
