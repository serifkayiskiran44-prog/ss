...
public static class JoomPanel
{
    ...
    {
        var store = new JoomSettingsStore(directory is null ? null : System.IO.Path.Combine(directory, "joom.bin")); var root = new StackPanel { Margin = Spacing.Page, MaxWidth = 820 };
        root.Children.Add(new TextBlock { Text = "Joom bağlantısı", FontSize = DesignTokens.TextSectionTitleSize, FontWeight = DesignTokens.FontWeightTitle, Margin = new Thickness(0, 0, 0, 10) }); root.Children.Add(new TextBlock { Text = "Merchant ID ve API key DPAPI ile şifrelenir. Güncel Joom API sözleşmesi doğrulanmadan canlı veri değişmez.", TextWrapping = TextWrapping.Wrap, Foreground = Brushes.DarkSlateGray, Margin = Spacing.BelowSection });
    ...
    static Button Button(string text, Action action) { var b = new Button { Content = text, Margin = Spacing.Control, Padding = DesignTokens.CompactButtonPadding }; b.Click += (_, _) => { try { action(); } catch (Exception e) { MessageBox.Show(MarketplaceConnectionStore.Redact(e.Message), "Joom", MessageBoxButton.OK, MessageBoxImage.Warning); } }; return b; }
