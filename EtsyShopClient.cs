...
{
    public async Task ValidateShopCurrencyAsync(EtsyCredentials credentials,string expected,CancellationToken cancellationToken=default)
    {
        ...
        using var request=new HttpRequestMessage(HttpMethod.Get,$"https://openapi.etsy.com/v3/application/shops/{shopId.ToString(CultureInfo.InvariantCulture)}"); EtsyHttp.AddHeaders(request,credentials,true);
        using var document=await EtsyHttp.SendJsonAsync(client,request,1024*1024,cancellationToken).ConfigureAwait(false);
        var remote=document.RootElement.TryGetProperty("currency_code",out var value)?value.GetString():null;
        if(string.IsNullOrWhiteSpace(remote)||!string.Equals(remote,expected,StringComparison.OrdinalIgnoreCase))throw new InvalidOperationException("CURRENCY_MISMATCH: Etsy mağaza dövizi önizleme ile eşleşmiyor; PATCH engellendi.");
    }
    ...
    public async Task<EtsyListingUpdateResult> UpdateSimpleListingAsync(EtsyCredentials credentials,long listingId,int quantity,decimal price,CancellationToken cancellationToken=default)
    {
        ...
        if(!long.TryParse(credentials.ShopId,NumberStyles.None,CultureInfo.InvariantCulture,out var shopId)||shopId<=0)throw new ArgumentException("Mağaza kimliği pozitif bir sayı olmalıdır.");
        using var request=new HttpRequestMessage(new HttpMethod("PATCH"),$"https://openapi.etsy.com/v3/application/shops/{shopId}/listings/{listingId.ToString(CultureInfo.InvariantCulture)}");EtsyHttp.AddHeaders(request,credentials,true);var values=new Dictionary<string,string>{{"quantity",quantity.ToString(CultureInfo.InvariantCulture)},{"price",price.ToString("0.00##########################",CultureInfo.InvariantCulture)}};if(quantity==0)values["state"]="inactive";request.Content=new FormUrlEncodedContent(values);
