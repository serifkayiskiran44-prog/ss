...
public static class AmazonPanel
{
    ...
    static Button Button(string text, Action action) { var b = new Button { Content = text, Margin = new Thickness(3), Padding = new Thickness(10, 5, 10, 5) }; b.Click += (_, _) => { try { action(); } catch (Exception e) { MessageBox.Show(MarketplaceConnectionStore.Redact(e.Message), "Amazon", MessageBoxButton.OK, MessageBoxImage.Warning); } }; return b; }
