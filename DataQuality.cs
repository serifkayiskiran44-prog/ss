...
public sealed class DataQualityService
{
    ...
    public IReadOnlyList<DataQualityIssue> Scan()
    {
        ...
        try { var coverage = new CategoryAttributeRuleStore(directory).Snapshot(); foreach (var product in products) { var verdict = coverage.Evaluate(product); if (!verdict.Blocks) continue; if (verdict.Missing.Count > 0) Add(new() { Severity = "Error", Type = "MissingRequiredAttribute", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = "Zorunlu özellik eksik: " + string.Join(", ", verdict.Missing) + ".", Suggestion = "Ürün kartındaki Özellikler alanına ad=değer biçiminde girin." }); foreach (var bad in verdict.Invalid) Add(new() { Severity = "Error", Type = "InvalidAttributeValue", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = $"Özellik değeri izinli değil: {bad.Attribute} = {bad.Value} (izinli: {bad.Allowed}).", Suggestion = "Sözlükteki izinli değerlerden birini seçin veya sözlüğe yeni değer ekleyin." }); } } catch { }
