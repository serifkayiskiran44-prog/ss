...
public sealed class DataQualityService
{
    ...
    public IReadOnlyList<DataQualityIssue> Scan()
    {
        ...
        foreach (var product in products)
        {
            ...
            if (product.Currency is not ("TRY" or "USD" or "EUR" or "GBP")) Add(new() { Severity = "Error", Type = "InvalidCurrency", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = $"Desteklenmeyen döviz: {product.Currency}.", Suggestion = "TRY, USD, EUR veya GBP seçin." });
        ...
        // #914: a product's category against its channel mappings, per plan -- an inactive category blocks (Error); a category the dictionary does not know, a missing mapping or a label changed after the mapping warns; a mapping whose category is gone is named by its external key.
        try { foreach (var group in plans.List().GroupBy(p => (Channel: p.ChannelId.ToLowerInvariant(), Shop: p.ShopId))) { var health = CategoryMappingHealth.Snapshot(directory, group.Key.Channel, group.Key.Shop); foreach (var orphan in health.Orphans) Add(new() { Severity = "Error", Type = "StaleCategoryMapping", Marketplace = group.Key.Channel, ShopId = group.Key.Shop, Message = $"Kanal kategori eşlemesi silinmiş bir yerel kategoriye bağlı (harici anahtar: {orphan.ExternalKey}).", Suggestion = "Eşlemeyi kaldırın veya var olan bir kategoriye yeniden bağlayın." }); foreach (var plan in group) { var product = products.FirstOrDefault(x => x.Id == plan.ProductId); if (product is null) continue; var verdict = health.Evaluate(product.Category); if (verdict.Status is CategoryMappingVerdict.Ok or CategoryMappingVerdict.NoCategory) continue; Add(new() { Severity = verdict.Blocks ? "Error" : "Warning", Type = "StaleCategoryMapping", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Marketplace = plan.ChannelId, ShopId = plan.ShopId, Message = "Kategori eşlemesi: " + verdict.Words, Suggestion = verdict.Blocks ? "Kategoriyi etkinleştirin veya ürünü başka kategoriye taşıyın; ilan gönderimi bu düzelmeden engellenir." : "Kategori sözlüğünü ve kanal eşlemesini doğrulayın." }); } } } catch { }
        // #915: attribute completeness per category -- a required attribute missing or a value outside the attribute's allowed values makes the product incomplete for listing; a category without rules, no category or an unknown one has nothing to cover.
        try { var coverage = new CategoryAttributeRuleStore(directory).Snapshot(); var unmappedValues = new List<AttributeValueSighting>(); /* #916 */ foreach (var product in products) { var verdict = coverage.Evaluate(product); if (!verdict.Blocks) continue; if (verdict.Missing.Count > 0) Add(new() { Severity = "Error", Type = "MissingRequiredAttribute", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = "Zorunlu özellik eksik: " + string.Join(", ", verdict.Missing) + ".", Suggestion = "Ürün kartındaki Özellikler alanına ad=değer biçiminde girin." }); foreach (var bad in verdict.Invalid) { unmappedValues.Add(new AttributeValueSighting(bad.Attribute, bad.Value, product.Sku)); Add(new() { Severity = "Error", Type = "InvalidAttributeValue", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = $"Özellik değeri izinli değil: {bad.Attribute} = {bad.Value} (izinli: {bad.Allowed}).", Suggestion = "Sözlükteki izinli değerlerden birini seçin veya sözlüğe yeni değer ekleyin." }); } } if (unmappedValues.Count > 0) new AttributeValueMappingStore(directory).Record(unmappedValues, DateTime.UtcNow); /* #916: the values the coverage could not place, queued with their evidence */ } catch { }
