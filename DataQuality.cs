...
public sealed class DataQualityService
{
    ...
    public IReadOnlyList<DataQualityIssue> Scan()
    {
        ...
        foreach (var group in products.Where(x => !string.IsNullOrWhiteSpace(x.Sku)).GroupBy(x => x.Sku, StringComparer.OrdinalIgnoreCase).Where(x => x.Count() > 1)) foreach (var product in group) Add(new() { Severity = "Critical", Type = "DuplicateSku", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = $"SKU {product.Sku} birden fazla üründe kullanılıyor.", Suggestion = "SKU'yu tek üründe bırakın veya güvenli bir düzeltme preview'i hazırlayın." });
        foreach (var group in products.Where(x => !string.IsNullOrWhiteSpace(x.Barcode)).GroupBy(x => x.Barcode, StringComparer.OrdinalIgnoreCase).Where(x => x.Count() > 1)) foreach (var product in group) Add(new() { Severity = "Critical", Type = "DuplicateBarcode", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = $"Barkod {product.Barcode} birden fazla üründe kullanılıyor.", Suggestion = "Barkod eşlemesini düzeltmeden sync başlatmayın." });
        foreach (var product in products)
        {
            ...
            foreach (var url in product.ImageUrls.Split('|', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries)) if (!Uri.TryCreate(url, UriKind.Absolute, out var uri) || uri.Scheme is not ("https" or "http" or "file")) Add(new() { Severity = "Warning", Type = "InvalidImageUrl", ProductId = product.Id, Sku = product.Sku, SourceId = product.SourceId, Message = "Görsel URL'si geçersiz veya desteklenmeyen şemada.", Suggestion = "HTTPS veya yerel file URL kullanın." });
        }
