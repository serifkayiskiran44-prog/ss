...
public static class ProductQuickInspect
{
    ...
    public static ProductQuickInspectView Build(CatalogProduct product, IReadOnlyList<SyncJob> jobs, DateTime nowUtc, IReadOnlyList<XmlSource>? sources = null, IReadOnlyList<OrderSnapshot>? orders = null, IReadOnlyList<SourceStockObservation>? stockObservations = null)
    {
        ...
        if (stockObservations is not null) Add("Stok", "Çoklu kaynak stoku", MultiSourceStock.Aggregate(product, sources ?? Array.Empty<XmlSource>(), stockObservations, nowUtc).Words); // #935: every source's last reported stock, judged and never double-counted
