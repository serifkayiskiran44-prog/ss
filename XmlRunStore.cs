...
public sealed class XmlRunStore
{
    ...
    public string Start(string sourceId, string feedHash, TimeSpan? lease)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = "INSERT INTO XmlRuns(Id,SourceId,Status,StartedUtc,LeaseUntilUtc,FeedHash) VALUES($id,$source,'Running',$started,$lease,$hash)";
    ...
    public IReadOnlyList<XmlRunRecord> List(string? sourceId = null, int limit = 100)
    {
        ...
        command.CommandText = sourceId is null
            ? "SELECT Id,SourceId,Status,StartedUtc,FinishedUtc,Added,Updated,Unchanged,Error,LeaseUntilUtc,FeedHash FROM XmlRuns ORDER BY StartedUtc DESC LIMIT $limit"
            : "SELECT Id,SourceId,Status,StartedUtc,FinishedUtc,Added,Updated,Unchanged,Error,LeaseUntilUtc,FeedHash FROM XmlRuns WHERE SourceId=$source ORDER BY StartedUtc DESC LIMIT $limit";
        ...
        while (reader.Read())
        {
            rows.Add(new XmlRunRecord(
                ...
                reader.GetInt32(5), reader.GetInt32(6), reader.GetInt32(7), reader.GetString(8), reader.IsDBNull(9) ? null : ParseUtc(reader.GetString(9)), reader.GetString(10)));
        }
        return rows;
    }
