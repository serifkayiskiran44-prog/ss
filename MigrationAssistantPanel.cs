...
public static class MigrationAssistantPanel
{
    ...
    {
        var service = new MigrationAssistantService(directory); MigrationPreview? preview = null; var root = new DockPanel { Margin = Spacing.Section }; var header = new StackPanel(); DockPanel.SetDock(header, Dock.Top); root.Children.Add(header);
        header.Children.Add(Text("Güvenli veri geçiş asistanı", TextRole.SectionTitle)); header.Children.Add(Text("Excel, CSV/TSV, JSON ve XML dışa aktarımlarını önce şema ve satır bazında önizler. Credential/token/parola alanları okunmaz; canlı marketplace değişikliği yapılmaz."));
        ...
        var sourceId = new TextBox { Text = "migration", Width = 150 }; controls.Children.Add(Text("Kaynak")); controls.Children.Add(sourceId); var analyze = Button("Şemayı analiz et / önizle"); controls.Children.Add(analyze); var apply = Button("Yedek al ve uygula"); controls.Children.Add(apply); var undoId = new TextBox { Width = 220, ToolTip = "Geçiş günlüğü ID" }; controls.Children.Add(undoId); controls.Children.Add(Button("Geçişi geri al", () => { service.Undo(undoId.Text.Trim()); status.Text = "Seçilen geçiş geri alındı."; })); header.Children.Add(controls);
        header.Children.Add(status);
        var grid = new DataGrid { AutoGenerateColumns = false, IsReadOnly = true, EnableRowVirtualization = true, Height = 430 }; foreach (var column in new[] { ("Satır", "RowNumber", 60d), ("Karar", "Action", 80d), ("SKU", "Product.Sku", 130d), ("Ürün", "Product.Name", 260d), ("Barkod", "Product.Barcode", 130d), ("Hata", "Error", 450d) }) grid.Columns.Add(GridColumns.Text(column.Item1, column.Item2, column.Item3)); root.Children.Add(grid);
    ...
    static Button Button(string text, Action? action = null) { var button = new Button { Content = text, Margin = Spacing.Control }; if (action is not null) button.Click += (_, _) => { try { action(); } catch (Exception error) { MessageBox.Show(MarketplaceConnectionStore.Redact(error.Message), "Veri geçişi", MessageBoxButton.OK, MessageBoxImage.Warning); } }; return button; }
}
