...
public static class ReportsPanel
{
    ...
    public static FrameworkElement Create(string? directory, Action<string>? navigate = null, Func<IReadOnlyCollection<string>>? allowedStoreKeys = null, Func<ReportDefinition, string?>? choosePath = null)
    {
        ...
        void Select(ReportCard card)
        {
            ...
            Func<ReportParameterSet, Task<string?>>? run = ReportRunner.CanRun(definition) ? async parameters =>
            {
                var path = choosePath is not null ? choosePath(definition) : AskPath(panel, definition);
                if (string.IsNullOrWhiteSpace(path)) return null;
                var outcome = await ReportRunner.RunAsync(directory, definition, parameters, path, allowedStoreKeys?.Invoke(), CancellationToken.None);
                Refresh();
