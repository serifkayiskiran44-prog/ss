...
- Product matching uses exact saved integration barcode, exact local barcode or exact remote stockCode/SKU. Conflicting candidates are errors, not guessed matches. Blank local barcode is allowed.
...
## Verification
- Final command: `dotnet test tests\MarketplaceHub.Tests\MarketplaceHub.Tests.csproj -c Release`: **1038 passed, 0 failed, 0 skipped**.
